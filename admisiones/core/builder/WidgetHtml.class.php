...
//Fin del método session
    ...
    function cuadro_lista($atributos) {
        ...
        if ($resultado) {
            ...
            if (isset($atributos["validar"]) && $atributos["validar"] != "") {
            ...
            if ($seleccion == -1) {
                $this->cadena_html.="<option value='-1' selected> </option>\n";
            }
        ...
        for ($j = 0; $j < $this->cuadro_campos; $j++) {
            ...
            if ($seleccion < 0 && $j == 0) {
                if ($limitar == 1) {
                    ...
                    $this->cadena_html.="<option value='" . $this->cuadro_registro[$j][0] . "'>" . stripslashes(html_entity_decode($this->cuadro_registro[$j][1])) . "</option>\n";
                }
            } else {
                if ($limitar == 1) {
                    ...
                    if ($this->cuadro_registro[$j][0] == $seleccion) {
                        $this->cadena_html.="<option value='" . $this->cuadro_registro[$j][0] . "' selected='true'>" . stripslashes(html_entity_decode($this->cuadro_registro[$j][1])) . "</option>\n";
                    } else {
                        $this->cadena_html.="<option value='" . $this->cuadro_registro[$j][0] . "'>" . stripslashes(html_entity_decode($this->cuadro_registro[$j][1])) . "</option>\n";
                    }
                }
        ...
        if (isset($miniRegistro)) {
            for ($i = 0; $i < $totalMiniRegistro; $i++) {
                ...
                if ($seleccion < 0 && $i == 0) {
                    ...
                    elseif($limitar=2){
                        if ($miniRegistro[$i][0] == $seleccion) {
                            $this->cadena_html.="<option class='texto_negrita' value='" . $atributos["valor"] . "' selected='true'>" . htmlentities($atributos["valor"]) . "</option>\n";
                            $seleccion = time();
                        } else {
                ...
                foreach ($opciones as $clave => $valor) {
                    ...
                    if ($opcion[0] != "") {
                        if ($opcion[0] != $atributos["seleccion"]) {
                            $this->miOpcion.="<div>";
                            $this->miOpcion.="<input type='radio' class='validate[required] radio' id='" . $id . "' name='" . $nombre . "' value='" . $opcion[0] . "' />";
