            ...
            {    
               ...
               $registroPreguntas = $esteRecursoDB->ejecutarAcceso($cadena_sql, "busqueda");
                ...
                foreach ($_REQUEST as $clave => $valor) {
                    //echo $clave ."=>". $valor."<br>";
                    if($valor=='' && $clave!='true' && $clave!='observaciones')
                    {
                ...
                if($_REQUEST['prestentaPor']=='1')
                {
                    $presentaPor='Primera vez';
                }
                elseif($_REQUEST['prestentaPor']=='2')
                {
                ...
                {
                    if($registroTipIns[$i]['ti_id']==$_REQUEST['tipoInscripcion'])
                    {
                ...
                {
                    if($estrato[$i][0]==$_REQUEST['estratoCosteara'])
                    {
                ...
                if($_REQUEST['numSemestres']==0)
                {
                    $numSemestres='Recién graduado';
                }
                elseif($_REQUEST['numSemestres']==1)
                {
                    $numSemestres='1 Semestre';
                }
                elseif($_REQUEST['numSemestres']==2)
                {
                    $numSemestres='2 Semestres';
                }
                elseif($_REQUEST['numSemestres']==3)
                {
                    $numSemestres='3 Semestres';
                }
                elseif($_REQUEST['numSemestres']==4)
                {
                    $numSemestres='4 Semestres';
                }
                elseif($_REQUEST['numSemestres']==5)
                {
                    $numSemestres='5 Semestres';
                }
                elseif($_REQUEST['numSemestres']==6)
                {
                ...
                {
                    if($registroDiscapacidad[$i][0]==$_REQUEST['discapacidad'])
                    {
                ...
                $valorCodificado.="&medio=".$_REQUEST['medio'];
                $valorCodificado.="&prestentaPor=".$_REQUEST['prestentaPor'];
                ...
                $valorCodificado.="&estratoCosteara=".$_REQUEST['estratoCosteara'];
                ...
                $valorCodificado.="&tipoColegio=".$_REQUEST['tipoColegio'];
                $valorCodificado.="&valido=".$_REQUEST['valido'];
                $valorCodificado.="&numSemestres=".$_REQUEST['numSemestres'];
                $valorCodificado.="&discapacidad=".$_REQUEST['discapacidad'];
                ...
                echo "<tr>";        
                    ...
                    $atributos ["tamanno"]="pequenno";
                    $atributos["tipo"] = $tipo;
                    $atributos["mensaje"] = "<strong>".$_REQUEST['valido']."</strong>";
                    echo $this->miFormulario->campoMensaje($atributos);
                    unset($atributos);  
                    echo"</td>";
                echo "</tr>";
                ...
                {    
                    $directorio = opendir($rutaArchivos); //ruta actual

                    //DOCUMENTOS CARGADOS
                    $atributos["id"] = "marcoDocumentosAdjuntos";
                    $atributos["estilo"] = "jqueryui";
                    $atributos["leyenda"] = "Lista de documentos adjuntos.";
                    echo $this->miFormulario->marcoAgrupacion("inicio", $atributos);
                    unset($atributos);
                    ...
                    while ($archivo = readdir($directorio)) //obtenemos un archivo y luego otro sucesivamente
                    {
                        if ($archivo!='..' && $archivo!='.')
                        {
                            ...
                            if(trim($_REQUEST['rba_id'])==$dividimos[1])
                            {
                                if(trim($_REQUEST['carreras'])==$dividimos[2])
                                {
                                    $variable['carrera']=$_REQUEST['carreras'];
                    ...
                    {    
                        ...
                        $atributos ["tamanno"]="pequenno";
                        $atributos["tipo"] = $tipo;
                        $atributos["mensaje"] = "Para esta carrera no aplica...";
                        echo $this->miFormulario->campoMensaje($atributos);
                        unset($atributos);
                    }        
                    echo $this->miFormulario->marcoAGrupacion("fin");
