...
if($cierto==1)
{
    ...
    $valorCodificado.="&opcion=guardarPdfResultadosEspeciales";
    ...
    $atributos["leyenda"] = "Cargue de archivos PDF resultados inscripciones especiales";
    ...
    //---------------Inicio Formulario (<form>)--------------------------------
        ...
        unset($atributos);
         ...
         //-------------Control Mensaje-----------------------
        $tipo = 'message';
        $mensaje = "EL SIGUIENTE FORMLUARIO LE PERMITIRÁ SUBIR LOS PDF CON LOS RESULTADOS DE INSCRIPCIONES ESPECIALES,
                ...
                while ($archivo = readdir($directorioArchivos)) //obtenemos un archivo y luego otro sucesivamente
                {
                    if ($archivo!='..' && $archivo!='.')
                    {
                        ...
                        if(trim($variable['anio'])==trim($dividimos[2]) && trim($variable['periodo'])==trim($dividimos[3]))
                        {
                            ...
                            if(trim($dividimos[1])=="especiales")
                            {
