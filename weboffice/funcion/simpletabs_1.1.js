...
var simpleTabs = {
	...
	init: function(){
		...
		var containerDiv = document.getElementsByTagName("tdiv");
		...
		for(var i=0; i<containerDiv.length; i++){
			if (containerDiv[i].className == "simpleTabs") {
				...
				// Tab Content
				var tdiv = containerDiv[i].getElementsByTagName("tdiv");
				
				//alert(tdiv.length);
				for(var l=0; l<tdiv.length; l++){
					
					if (tdiv[l].className == "simpleTabsContent") {
						tdiv[l].setAttribute("id",containerDivId+"_tdiv_"+[l]);	
						if(simpleTabs.readCookie('simpleTabsCookie')){
							var cookieElements = simpleTabs.readCookie('simpleTabsCookie').split("_");
							var curTabCont = cookieElements[1];
							var curAnchor = cookieElements[2];		
							if(tdiv[l].parentNode.getAttribute("id")=="tabber"+curTabCont){
								if(tdiv[l].getAttribute("id")=="tabber"+curTabCont+"_tdiv_"+curAnchor){
									tdiv[l].className = "simpleTabsContent currentTab";
								} else {
									tdiv[l].className = "simpleTabsContent";
								}
							} else {
								tdiv[0].className = "simpleTabsContent currentTab";
							}
						} else {
							tdiv[0].className = "simpleTabsContent currentTab";
						}
	...
	setCurrent: function(elm,cookie){
		...
		// change to associated tdiv
		var otherDivs = document.getElementById(thisContainerID).getElementsByTagName("tdiv");
		for(var i=0; i<otherDivs.length; i++){
			if ( /simpleTabsContent/.test(otherDivs[i].className) ) {
				otherDivs[i].className = "simpleTabsContent";
			}
		}
		document.getElementById(thisContainerID+"_tdiv_"+thisLinkPosition).className = "simpleTabsContent currentTab";
