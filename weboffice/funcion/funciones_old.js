...
/*************************************************************
*Funcion que solo permite el ingreso de datos numericos
**************************************************************/
...
/*************************************************************
*Funcion que solo permite el ingreso de solo datos alfanumericos
**************************************************************/
...
function fecha(formulario,fecha_for,fecha2){
	...
	if (f1>f2) 
			{campo.select();
            alert1('No es posible Guardar con la fecha '+f_inicial+', ya que es es mayor a la del día de hoy '+fecha2);
	        campo.focus();
	        return false;
			}
		return true;	
...
/***************************************************************************************************
*Funcion para la impresión parcial de una página
***************************************************************************************************/
...
/******************************************************************************************************
*Funcion para llamar una ventana cuando se hace click sobre un objeto y retorne unos valores
******************************************************************************************************/	
var WinOpen=0;
...
/*********************************************************************************************************
*Funcion para llamar n mensaje cuando el cursor se ubique sobre un objero
**********************************************************************************************************/
var theObj="";
...
function addModulo(formulario, control1, select_control2, select_control3, string){

    var indice=select_control2.selectedIndex;
    var nom=select_control2.options[indice].text;
    var idMod=select_control2.options[indice].value;

    var indice2=select_control3.selectedIndex;
    var nom2=select_control3.options[indice2].text;
    var idnom2=select_control3.options[indice2].value;
    ...
    var datos=(control1.getElementsByTagName("tr").length);

    for (var i=1; i<datos; i++){
        var name="idTar"+i;
        var tarId=document.getElementById(name).value;
        var seleccion=idMod+"_"+idnom2;
        if(tarId==seleccion){
            alert("El registro ya fue seleccionado para el usuario");
            var bandera=1;
        }
    }


    if (bandera != 1 || datos == 0){
        encabezado1.innerHTML="<color='red' border='2'><br>modulo";
        encabezado2.innerHTML="<br>Tarea";

    var nombre="consecutivo1_1";
    document.getElementById(nombre).value=datos;

     if (datos==0){
        celda1.innerHTML="<input class='sigma' type='text' value='"+nom+"' readonly='readonly' name ='text_modulo"+((control1.getElementsByTagName("tr").length))+"' size='20%'>\n\
        <input type='hidden' name='idMod"+(datos+1)+"' value='"+idMod+"' id='idmodulo"+(datos+1)+"'>\n\
        <input type='hidden' name='valor_mod' value='"+(datos+1)+"' ";

        celda2.innerHTML="<input type='text' value='"+nom2+"' readonly='readonly' name ='text_tarea"+((control1.getElementsByTagName("tr").length))+"' size='45%'><input type='button' name='del' value='-' onclick='delCarrera("+string+","+((control1.getElementsByTagName("tr").length)+1)+")'>\n\
        <input type='hidden' name='idTar"+((datos)+1)+"' value='"+idMod+"_"+idnom2+"' id='idTar"+((datos)+1)+"'>\n\
        <input type='hidden' name='valor_tar' value='"+((datos)+1)+"'>";
     }
     else {

        celda1.innerHTML="<input class='sigma' type='text' value='"+nom+"' readonly='readonly' name ='text_modulo"+((control1.getElementsByTagName("tr").length))+"' size='20%'>\n\
        <input type='hidden' name='idMod"+(datos)+"' value='"+idMod+"' id='idmodulo"+(datos)+"'>\n\
        <input type='hidden' name='valor_mod' value='"+(datos)+"' ";

        celda2.innerHTML="<input type='text' value='"+nom2+"' readonly='readonly' name ='text_tarea"+((control1.getElementsByTagName("tr").length))+"' size='45%'><input type='button' name='del' value='-' onclick='delCarrera("+string+","+((control1.getElementsByTagName("tr").length))+")'>\n\
        <input type='hidden' name='idTar"+((datos))+"' value='"+idMod+"_"+idnom2+"' id='idTar"+(datos)+"'>\n\
        <input type='hidden' name='valor_tar' value='"+((control1.getElementsByTagName("tr").length))+"' ";
     }
    ...
    if(datos==0){
        ...
        control1.appendChild(fila1);
        control1.appendChild(fila2);
    }
    else{
...
function delCarrera(control1, fila){
    //alert("entra al del ");
    control1.deleteRow(fila);
}
