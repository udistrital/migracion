...
class navegacion
{
	...
	?><table width="100%" align="center" border="0" cellpadding="10" cellspacing="0" >
		<tbody>
			<tr>
				<td >
					<table width="100%" cellpadding="2" cellspacing="2" style="font-size:12px; font-family:Arial,Verdana,Trebuchet MS,Helvetica,sans-serif; -moz-border-radius-bottomleft:10px; -moz-border-radius-bottomright:10px; background-color:#FFFFFF; border:1px solid #CCCCCC;">
					...
					?>	</td>
						...
						<?
                                                        $hoja=isset($_REQUEST['hoja'])?$_REQUEST['hoja']:'';
							if($hoja<$hojas)
							{
	...
	function formulario_navegacion($configuracion,$hojas,$mi_variable,$cripto="")
	{
		...
		$texto_ayuda="<b>Desplazarse entre p&aacute;ginas de resultados</b><br>Ingrese el n&uacute;mero de la p&aacute;gina a la que desea ir y presione la tecla ENTER.<br>Existe un total de <b>".$hojas."</b> p&aacute;gina(s).";
                $hoja=isset($_REQUEST['hoja'])?$_REQUEST['hoja']:'';
                $cadena_html.="Hoja  <input class='celdanavegacion' type='text' name='hoja' size='2' maxlength='6' value='".$hoja."' onmouseover='return escape(\"".$texto_ayuda."\")' onkeypress=\"if ((event.which == 13) || (event.keyCode == 13) || (event.which == 10) || (event.keyCode == 10)){if(".$verificar."){document.forms['".$formulario."'].submit()}else{return(false)}}\"> de ".($hojas)."\n";	
		$cadena_html.="</form>";
