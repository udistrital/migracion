...
class validarUsu {
   ...
   * @return string
   */  
   public function validarProyectoAsistente($codEstudiante, $codAsistente,$conexion,$configuracion,$accesoOracle,$nivel)
    {
        if(is_numeric($codEstudiante))
        {
          $cadena_sql=$this->cadena_sql("buscarInfoEstudiante",$codEstudiante); 
          $resultado_estudiante = $this->funcion->ejecutarSQL($configuracion,$conexion,$cadena_sql,"busqueda");
        
          if(is_array($resultado_estudiante))
            {
                $mensaje=  $this->verificarProyectoAsistente($resultado_estudiante[0][1],$codAsistente,$nivel,$conexion,$configuracion,$accesoOracle);
     ...
     * @return type
     */
    function consultarProyectosAsistente($codAsistente,$tipoUsuario,$conexion,$configuracion,$acceso){
        $datos=array('codAsistente'=>$codAsistente,
                     'tipoUsuario'=>$tipoUsuario);
        $cadena_sql_proyectos=$this->cadena_sql("proyectos_curriculares_asistente",$datos);
        $resultado_proyectos = $this->funcion->ejecutarSQL($configuracion,$conexion,$cadena_sql_proyectos,"busqueda");
