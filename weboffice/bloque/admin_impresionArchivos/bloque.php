...
$valor[100]=$_REQUEST["usuario"];
$valor[1]=$anno;
$valor[2]=$periodo;
$valor[3]=$_REQUEST["carrera"];
//echo "mmm".$valor[3]."<br>";

$cadena_sql=cadena_busqueda_recibo($configuracion, $accesoOracle, $valor,"recibosGenerados");
	//echo $cadena_sql.'<br>';
$registroGenerados=ejecutar_admin_recibo($cadena_sql,$accesoOracle,"busqueda");
$cuentaRecibos=count($registroGenerados);
//echo "mmm".$cuentaRecibos;
for($i=0; $i<=$cuentaRecibos; $i++)
{
	//set_time_limit(800);
	$valor[0]=$registroGenerados[$i][0];
	$valor[4]=$registroGenerados[$i][1];
	...
	else
	{
		...
		$cadena_sql=cadena_busqueda_recibo($configuracion, $accesoOracle, $valor,"cambiaaP");
...
if($cierto==1)
{
	...
	alert('<?echo 'Archivos procesados exitosamente!'?>');
	</script>                   
	<?
	...
	//echo "<script>parent.location.href='".$enlace."'</script>";
}
/****************************************************************
*  			Funciones				*
****************************************************************/
...
function con_registro_recibo($configuracion,$registro,$campos,$tema,$acceso_db,$accesoOracle,$datoBasico)
{
	...
	//$factura=isset($factura)?$factura:null;
	$factura=explode(",",$_REQUEST["factura"]);
	//$valor[0]=$factura;
	$cuentaRecibos=count($factura);
	//echo "mmm".$cuenta."<br>";
	$valor[1]=$anno;
	$valor[2]=$periodo;
	...
	$pdf->Output('recibos/'.$valor[4].''.$parametro[0].'.pdf','F');

	//echo "prueba";
	//$variable=$_REQUEST["usuario"]."<br>";
	/*$indicedoc=$configuracion["host"].$configuracion["site"]."/index.php?";
	$variable="pagina=adminConsultaRecibos";
	//Codigo del Estudiante
...
function cadena_busqueda_recibo($configuracion, $acceso_db, $valor,$opcion="")
{
	...
	switch($opcion)
	{
		case "recibosGenerados":
			...
			$cadena_sql.="ACCRA ";
			$cadena_sql.="WHERE ";
			$cadena_sql.="ema_cra_cod=".$valor[3]." ";
			$cadena_sql.="AND ";
			...
			$cadena_sql.="AND ";
			$cadena_sql.="ema_enviado_email='N' ";
			$cadena_sql.="AND ";
			$cadena_sql.="ema_est_cod NOT IN (SELECT deu_est_cod FROM acdeudores) ";
			$cadena_sql.="AND ";
			$cadena_sql.="ema_est_cod NOT IN (SELECT est_cod FROM acest WHERE est_estado_est IN ('J','B','W','Z','U') and est_cra_cod=".$valor[3].") ";
		...
		case "cambiaaP":
		      ...
		      $cadena_sql.="WHERE ";
		      $cadena_sql.="ema_est_cod =".$valor[4]." ";
		      $cadena_sql.="AND ";
		      $cadena_sql.="ema_cra_cod=".$valor[3]." ";
		      $cadena_sql.="AND ";
		      $cadena_sql.="EMA_ANO= ".$valor[1]." ";	
		      ...
		      $cadena_sql.="EMA_ESTADO='A' ";
		      $cadena_sql.="AND ";
		      $cadena_sql.="ema_est_cod NOT IN (SELECT deu_est_cod FROM acdeudores WHERE deu_est_cod =".$valor[4].") ";
		      $cadena_sql.="AND ema_est_cod NOT IN (SELECT est_cod FROM acest WHERE est_cod =".$valor[4]." AND est_estado_est IN ('J','B','W','Z','U') and est_cra_cod=".$valor[3].") ";
//		      $cadena_sql.="    ema_est_cod NOT IN (SELECT est_cod FROM acest WHERE est_cod =".$valor[4]." AND est_estado_est IN ('J','B')) ";
		      break;
