...
class funciones_registroLoteRecibo extends funcionGeneral
{
	...
	function __construct($configuracion, $sql)
	{
      ...
      function verificaCalendario($configuracion,$carrera,$evento){
	  
	  $variable["carrera"]=$carrera;
	  $variable["evento"]=$evento;
	  $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"verificaCalendario",$variable);
											...
											<table width="80%" align="center" border="0">
												<tr>
													<td align="center">
														...
														$datos="&pagina=registro_recibo_lote";
														$datos.="&procesar=true";
...
//cambiar periodo
//		$this->solicitud["periodo"]=3;
		$this->solicitud["unidad"]=0;
                    ...
                    $carrerasCoordinador=$this->carrerasCoordinador($configuracion, $this->identificacion, $this->accesoOracle);
                        ...
                        $proyectos =$this->validacion->consultarProyectosAsistente($this->usuario,  $this->nivel, $this->accesoOracle,$configuracion, $this->acceso_db);
                         foreach ($proyectos as $key => $proyecto) {
                            $carrerasCoordinador[$key][0]= $proyecto[0];
                            $carrerasCoordinador[$key][1]= $proyecto[4];
                         }
                }
	...
	function procesarRegistro($configuracion, $registro )
	{
		...
		//verifica el evento 82 que corresponde a fechas de generacion de recibos de terminacion de materias
		$fechasTerminacion=$this->verificaCalendario($configuracion,$this->solicitud["carrera"],82);
	

		if($fechasTerminacion=='S')
		{
			$cadena_sql=$this->sql->cadena_sql($configuracion, $this->accesoOracle,"datosBasicosEstudiante", $this->solicitud["estudiante"]);	
			$estudiante=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql ,"busqueda");
			if(is_array($estudiante))
			{
				if($estudiante[0][5]<>'T' && $estudiante[0][5]<>'H'){
					$this->errorProceso="El estado del estudiante es diferente de terminaci&oacute;n de materias";
					return false;
				}
			}			
		}
	...
	function verificarCodigo($configuracion, $carrera)
	{
		...
		if(is_array($pagador))
		{
			$estadoEstudiante=$pagador[0][5];

			if($estadoEstudiante<>'V' && $estadoEstudiante<>'J' && $estadoEstudiante<>'T' && $estadoEstudiante<>'A' && $estadoEstudiante<>'Q')
			{
				//echo "El estado actual del estudiante ".$pagador[0][0]." no permiten la generaci&oacute;n del recibo.<br>";
				$this->errorProceso="El estado del estudiante no permite la generaci&oacute;n del recibo de pago.";
				return false;
			}
	...
	function guardarCuota($configuracion,$registro='')
	{
...
//cambiar periodo
		...
		$cadena_sql=$this->sql->cadena_sql($configuracion, $this->accesoOracle,"facultadCarrera", $this->solicitud['carrera']);
		...
		switch($this->solicitud["plantilla"])
		{
			...
			case 1:
                                //Si en la plantilla esta marcado por terminacion de materias, busca las fechas de la cuota 3 de la facultad
                                $cuota=$this->solicitud["cuotaGuardar"];                                
                                if(isset($registro[8]))
                                {
                                    $this->solicitud["cuotaGuardar"]=3;
                                }
//Para la plantilla de pregrados se insertan las fechas para la generacion de recibos de 2014-3
				...
				if($this->solicitud["diferido"]=='S')
				{
					...
					switch($this->solicitud["cuota"])
					{
						case 0:
							...
							{
								$this->solicitud["cuotaGuardar"]=$i;
								$this->guardarCuota($configuracion,$registro);	
