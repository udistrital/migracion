...
class funciones_registroNotasDocentes extends funcionGeneral
{
	...
	function verListaClase($configuracion,$conexion)
	{
		if($this->usuario)
		{
								...
								$menu=new navegacion();
									if($resultLista[$i][12]=='PREGRADO' || $resultLista[$i][12]=='EXTENSION')
									{
										...
										echo "<td class='cuadro_plano centrar'>
											...
											$variable.="&grupo=".$resultLista[$i][10]."";
	...
	function digitarNotasPregrado($configuracion, $accesoOracle,$acceso_db)
	{
	...
	function digitarNotasPosgrado($configuracion, $accesoOracle,$acceso_db)
	{
		if($this->usuario)
		{
			$usuario=$this->usuario;
	...
	function guardarNotasPregrado($configuracion, $accesoOracle,$acceso_db)
	{
		if($this->usuario)
		{
			$usuario=$this->usuario;
		...
		foreach($_REQUEST as $clave=>$valor)
		{
			...
			if($clave==p1||$clave==p2||$clave==p3||$clave==p4||$clave==p5||$clave==p6||$clave==pl||$clave==pe)
			{
				...
				$acupor1 = $_REQUEST['p1']+$_REQUEST['p2']+$_REQUEST['p3']+$_REQUEST['p4']+$_REQUEST['p5']+$_REQUEST['p6']+$_REQUEST['pl'];
				$acupor2 = $_REQUEST['par1']+$_REQUEST['par2']+$_REQUEST['par3']+$_REQUEST['par4']+$_REQUEST['par5']+$_REQUEST['par6']+$_REQUEST['plab'];
		...
		for ($i=0; $i<=$resultverifica[0][32]; $i++)
		{
			...
			foreach($_REQUEST as $clave=>$valor)
			{
				...
				||(($valor==$_REQUEST[sprintf('nota_6%d', $i)]))
				||(($valor==$_REQUEST[sprintf('lab_1%d', $i)]))
				||(($valor==$_REQUEST[sprintf('exa_1%d', $i)]))
				||(($valor==$_REQUEST[sprintf('hab_1%d', $i)])))
				{
					//echo "aaa".$clave."->".$valor."<br>";
	...
	//Contiene las validaciones y las sentencias SQL que guarda las notas en el sistema.
	function guardarNotasPosgrado($configuracion, $accesoOracle,$acceso_db)
	{
		if($this->usuario)
		{
			$usuario=$this->usuario;
	...
	function calculoDefinitiva($configuracion, $accesoOracle,$acceso_db)
	{
		$valor[10]=$_REQUEST['periodo'];
		$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"anioper",$valor);
		$resultAnioPer=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
	...
	function notasPerAnterior($configuracion, $accesoOracle,$acceso_db)
	{
			...
			<tr class="formulario">
				...
				<?
					...
					if($fechahoy < $fecini || $fechahoy > $fecfin)
					{
						...
						while(isset($resultverifica[$i][0]))
						{
							echo '<td align="right">'.$resultverifica[$i][7].'</td>
								<td>'.$resultverifica[$i][8].'</td>
								<td align="center">'.$resultverifica[$i][10].'</td>
								<td align="center">'.$resultverifica[$i][12].'</td>
								<td align="center">'.$resultverifica[$i][14].'</td>
	...
	function fechasNotas($configuracion, $accesoOracle,$acceso_db)
	{
		$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"fechaactual",'');
		$rowfechoy=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
