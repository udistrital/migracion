...
if (is_resource($enlace))
{
	if(!isset($_REQUEST['cancelar']))
	{
		if(!isset($_REQUEST["confirmacion"]))
		{
			...
			if((!(strlen($_REQUEST['nombre'])>2)||!(strlen($_REQUEST['apellido'])>2)||!(strlen($_REQUEST['correo'])>6)||!(strlen($_REQUEST['usuario'])>2)||!(strlen($_REQUEST['clave'])>4)) && (!isset($_REQUEST['confirmacion'])))
			{
		...
		{
			confirmarUsuario($configuracion ,$acceso_db, $enlace);
		}
	}
	else
	{
		redireccionarUsuario($configuracion, "principal");	
	}
} 
else
{
...
*				Funciones						*
****************************************************************************************/
function confirmarUsuario($configuracion ,$acceso_db,$enlace)
{
	...
	if($campos>0)
	{
			$valor[0]=$registro[0][1];
			$valor[1]=$registro[0][2];
			$valor[2]=$registro[0][3];
			$valor[3]=$registro[0][4];
			$valor[4]=$registro[0][5];
			$valor[5]=$registro[0][6];
			$valor[6]=$registro[0][7];
			$valor[7]=$registro[0][8];
			$valor[8]=$registro[0][9];
			$valor[9]=$registro[0][10];
			$valor[10]=$registro[0][11];
			$valor[11]=$registro[0][12];
			$valor[12]=$registro[0][13];
			
			$cadena_sql=sqlRegistroUsuario($configuracion, "insertar",$valor);
			...
			if($resultado==TRUE)
			{
				...
				unset($valor);
				unset($registro);
				$cadena_sql=sqlRegistroUsuario($configuracion, "rescatarFormacionBorrador",$_REQUEST['identificador']);
				$campos=$acceso_db->registro_db($cadena_sql,0);
				$registro=$acceso_db->obtener_registro_db();
				if($campos>0)
				{
				
					$valor[0]=$ultimoInsertado;
					$valor[1]=$registro[0][1];
					$valor[2]=$registro[0][2];
					$valor[3]=$registro[0][3];
					$valor[4]=$registro[0][4];
					$valor[5]=$registro[0][5];
					$valor[6]=$registro[0][6];					
					$cadena_sql=sqlRegistroUsuario($configuracion, "insertarAcademica",$valor);		
					$resultado=$acceso_db->ejecutar_acceso_db($cadena_sql); 
					//exit;
...
function sqlRegistroUsuario($configuracion, $opcion, $valor)
{
	switch($opcion)
	{
		...
		case "rescatarFormacionBorrador":
			$cadena_sql="SELECT ";
			$cadena_sql.="`identificador`, ";
			...
			$cadena_sql.="WHERE ";
			$cadena_sql.="identificador=".$valor;	 
			break;
		...
		case "insertarAcademica":
			$cadena_sql="INSERT INTO ";
			$cadena_sql.=$configuracion["prefijo"]."registrado_formacion "; 
			$cadena_sql.="( ";
			$cadena_sql.="`id_usuario`, ";
			$cadena_sql.="`id_formacion`, ";
			$cadena_sql.="`id_area_desempenno`, ";
		...
		case "eliminarBorradorAcademica":
			$cadena_sql="DELETE ";
			$cadena_sql="FROM ";
			$cadena_sql.=$configuracion["prefijo"]."registrado_formacion_borrador "; 
			$cadena_sql.="WHERE "; 
...
function nuevoUsuario($configuracion,$acceso_db)
{
	...
	$cadena_sql=sqlRegistroUsuario($configuracion, "identificacion",$_REQUEST['identificacion']);
	$campos=$acceso_db->registro_db($cadena_sql,0);
	if($campos>0)
	{
	...
	if(isset($corregir))
	{
		...
		{
			$cadena_sql=sqlRegistroUsuario($configuracion, "eliminarBorrador",$_REQUEST["identificador"]);
			$acceso_db->ejecutar_acceso_db($cadena_sql); 
			$cadena_sql=sqlRegistroUsuario($configuracion, "eliminarBorradorAcademica",$_REQUEST["identificador"]);
			$acceso_db->ejecutar_acceso_db($cadena_sql); 
		}
		...
		{
			$valor[7]="Documento ya registrado";
		}
		...
		if($resultado==TRUE)
		{
			...
			$cadena_sql=sqlRegistroUsuario($configuracion, "insertarBorradorAcademica",$valor);		
			$resultado=$acceso_db->ejecutar_acceso_db($cadena_sql); 
			
			if($resultado==TRUE)
			{
				redireccionarUsuario($configuracion, "corregirUsuario",$identificador);
			}
