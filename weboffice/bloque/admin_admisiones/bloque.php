...
//Clase
class bloque_admin_admisiones extends bloque
{

	 public function __construct($configuracion)
	{
 		$this->sql=new sql_admin_admisiones();
 		$this->funcion=new funciones_admin_admisiones($configuracion, $this->sql);
	...
	function html($configuracion)
	{
		if(!isset($_REQUEST['cancelar']))
		{
			...
			if(isset($_REQUEST['opcion']))
			{
				...
				//echo "zzz".$_REQUEST["accion"]; exit;
				switch($accion)
				{
					case "presentacion":
						$this->funcion->presentacion($configuracion);
						break;
					case "adminFechasInsRes":
						$this->funcion->administracionFechasInsRes($configuracion);
						break;
				}
			}
			else
	...
	function action($configuracion)
	{
		...
		if(!isset($_REQUEST['cancelar']))
		{
			...
			if(isset($_REQUEST["actualizarFechasInscipciones"]))
			{
				$this->funcion->actualizarFechasIns($configuracion);
			}
		}
		else
		{
...
$esteBloque=new bloque_admin_admisiones($configuracion);
