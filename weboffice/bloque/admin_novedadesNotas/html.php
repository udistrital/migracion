...
{% block contenido_bloque2 %}
				...
				<form action='index.php' id='form_insertar' name='form_insertar'>
					...
					<table class="content_box">
						...
						<tr style='background:#FFFFFF'>
							...
							<td><input type='text' class="{digits: true, required: true, range: [0,99999]}" size='3' id="grupo" maxlenght="5" name="grupo" value="0"></td>
							<td><input type='text' class="{required: true, range: [0,99]}" size='2' id="semestre" name="semestre" ></td>
							<td><input type='text' class="{digits: true, required: true, range: [1948,2200]}" size='2' id="anio" name="anio" ></td>
							...
							<td>
								<select style="width:45px" id="obs" name="obs">
		...
		<table class="content_box" style="margin: auto;" >
			...
			{% for registro in notas %}
				...
				<form action='index.php'>
					...
					<tr id='reg_{{registro.9}}' style='background:#FFFFFF' onmouseover="this.style.background='#FFFFAA'" onmouseout="this.style.background=''">
						...
						<input type='hidden' size='2' id="carrera" name="carrera" value='{{ registro.21 }}'> <!--carrera-->
						...
						<input type='hidden' size='2' id="grupo_org" name="grupo_org" value='{{ registro.1 }}'> <!--grupo original-->
						<input type='hidden' size='3' id="obs_org" name="obs_org" value='{{ registro.7 }}'> <!--observacion original-->
						<input type='hidden' size='2' id="estado_org" name="estado_org" value='{{ registro.8 }}'> <!--estado original-->
						<input type='hidden' size='2' id="creditos_org" name="creditos_org" value='{{ registro.14 }}'>
						<input type='hidden' size='2' id="hteoricas_org" name="hteoricas_org" value='{{ registro.15 }}'>
						<input type='hidden' size='2' id="hpracticas_org" name="hpracticas_org" value='{{ registro.16 }}'>
						<input type='hidden' size='2' id="hautonomo_org" name="hautonomo_org" value='{{ registro.17 }}'>
						<input type='hidden' size='2' id="semestre_org" name="semestre_org" value='{{ registro.2 }}'>
						<input type='hidden' size='2' id="ceacod_org" name="ceacod_org" value='{{ registro.18 }}'> <!--clasificacion original-->
						...
						<td>{{ registro.21 }}</td>
						<td><input type='text' class="{digits: true, required: true, range: [0,99]}"  maxlength='2' size='2' id="semestre" name="semestre" value='{{ registro.2 }}'></td>
						<td>{{ registro.0 }}</td>
						<td>{{ registro.3 }}</td>
						<td><input type='text' class="{digits: true, required: true, range: [0,99999]}"  maxlength='5' size='3' id="grupo" name="grupo" value='{{ registro.1 }}'></td>
						<td>{{ registro.4 }}</td>
						<td>{{ registro.5 }}</td>
						<td>
							<input type='text' size='2' id="nota" maxlength='2' class="{digits: true, required: true, range: [0,50]}" name="nota" value='{{ registro.6 }}'>
						</td>
                                                    {% if notas.0.0.20=='PREGRADO' %}
                                                        {% if notas.0.0.13=='S' %}
                                                            ...
                                                            <td>
									<select style="width:45px" id="ceacod" name="ceacod">
                                                                                <option type='text' size='4' value='' selected>Sin clasificacion</option>
                                                                            {% for clasificacion in clasificaciones %}
                                                                                        {% if registro.18 == clasificacion.0 %}
                                                                                                <option type='text' size='4' value='{{clasificacion.0}}' selected>{{clasificacion.2}} - {{clasificacion.1}} </option>
                                                                                                
                                                                                        {% else %}
                                                                                                <option type='text' size='4' value='{{clasificacion.0}}' >{{clasificacion.2}} - {{clasificacion.1}} </option>
                                                                                        {% endif %}
	...
	{% endif %}
	<h3>Recomendaciones para el registro de la novedad:</h3>
	<ul class="tmo_list">
		<li>La novedad de nota se realiza únicamente para el proyecto actual del estudiante.</li>
		<li>Diligenciar todos los campos solicitados.</li>
	...
	<ul class="tmo_list">
        ...
        <h3>Recomendaciones para actualizar datos:</h3>
	<ul class="tmo_list">
		<li>En cualquier momento solo podrá actualizar los datos básicos de las notas como: Nivel, Grupo, HT, HP, HTD, HTC, AUT y Clasificación dependiendo si es de Horas o Créditos.</li>
		<li>Para actualizar datos de Valor de la Nota, Créditos, Estado y Observación, depende de las fechas establecidas para novedades de nota.</li>
