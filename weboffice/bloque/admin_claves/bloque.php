...
//Clase
class bloque_admin_claves extends bloque
{

	 public function __construct($configuracion)
	{
 		$this->sql=new sql_admin_claves();
 		$this->funcion=new funciones_admin_claves($configuracion, $this->sql);
			...
			if(isset($_REQUEST['opcion']))
			{
				...
				switch($accion)
				{
					case "presentacion":
						...
						break;
					case "enlace":
						$this->funcion->enviaEnlace($configuracion);
						break;
					case "cambiarClave";
					      $this->funcion->cambioClave($configuracion);
					      break;
					case "redireccionar";
					      $this->funcion->presentacion($configuracion);
					      break;
	...
	function action($configuracion)
	{
		...
		if(!isset($_REQUEST['cancelar']))
		{
			//echo $_REQUEST['proceso']; exit;
			$valor[3]=$_REQUEST['nivel'];
			$proceso=$_REQUEST['proceso'];
			switch($proceso)
			{
				case "confirmarDatos":
					$this->funcion->confirmaDatos($configuracion);
					break;
				case "cambiarClave":
					$this->funcion->modificarClave($configuracion);
					break;
			}
		}
		else
...
$esteBloque=new bloque_admin_claves($configuracion);
