...
* @author 		Milton Parra
* @description  Bloque principal para la administracion de recibos de pago
*
******************************************************************************/
...
{	
	...
	$cadena="<p>Por favor contacte a su Coordinador(a) para que generen su recibo a trav&eacute;s de plantillas</p>";
...
function con_registro_recibo($configuracion,$registro,$campos,$tema,$acceso_db,$accesoOracle,$datoBasico)
{
	...
	if(is_array($registroObservacion))
	{
		$valor[18]=$registroObservacion[0][12];
	}
	else
	{
		$valor[18]="";
...
function prepararPDF($configuracion,$pdf, $valor,$pago="",$tipo="",$conceptos="")
{
	...
	if($tipo=="proyecto" || $tipo=="banco")
	{
		...
		$pdf->Cell(40,4,utf8_decode("Francisco José de Caldas"),0);		
		$pdf->Ln(1);
		$pdf->Cell(70,4,"",0);
		//Comprobante de Pago
		...
		//Codigo Estudiante
		$pdf->Cell(26,4,utf8_decode("Código"),0);
		//Documento
		...
		//Fecha 1
		$pdf->Cell(50,4,utf8_decode("Fecha Límite"),0);
		//Pago Ordinario
		$pdf->Cell(50,4,"TOTAL A PAGAR",0);
		//Fecaha expedicion
		$pdf->Cell(40,4,utf8_decode("Fecha de Expedición"));
		//Periodo
		...
		$pdf->Cell(18,4,"Ref.",0);
		//Fecha expedicion
		$pdf->Cell(28,4,utf8_decode("Descripción"));
		//Periodo
		$pdf->Cell(20,4,"Valor");
		$pdf->Ln(5);
		...
		if(!is_array($conceptos))
		{
			...
			$pdf->Cell(5,4,"",0);
			//Descripcion
			$pdf->Cell(24,4,utf8_decode("Matrícula"),0);
			//Valor
	...
	else
	{
		...
		//Fecha expedicion
		$pdf->Cell(62,4,utf8_decode("Descripción"));
		//Periodo
		$pdf->Cell(45,4,"Valor");
		...
		if(!is_array($conceptos))
		{
			...
			$pdf->Cell(10,4,"001",0);
			//Descripcion
			$pdf->Cell(60,4,utf8_decode("Matrícula"),0);
			//Valor
