...
class funciones_registroLoteRecibo extends funcionGeneral
{
	...
	{
		$formulario="sec_registro_lote_recibo";
											...
											<table width="80%" align="center" border="0">
												<tr>
													<td align="center">
														...
														$datos="&pagina=sec_registro_recibo_lote";
														$datos.="&procesar=true";
...
__________________________________________________________________________________________________*/
	...
	function verificarLote($valor, $data, $tipo="", $configuracion)
	{
		...
		echo "<br>*9 Entro en funcion verificarlote el tipo de la plantilla es $tipo *  //funcion.class.php line 196";
		switch($tipo)
		{
			...
			case "PLANTILLA RECIBOS EXTEMPORANEOS PARA PREGRADOS":
				$filaInicio=5;			
				$codigoPlantilla=4;
				break;

			case "PLANTILLA RECIBOS EXTEMPORANEOS PARA POSGRADOS":
				$filaInicio=5;			
				$codigoPlantilla=5;
				break;
		...
		{
			echo "<br>*10 El coordinador ".$this->identificacion." si tiene carreras *  //funcion.class.php line 196";
	...
	function procesarRegistro($configuracion, $registro )
	{
		...
		//verifica si existe una solicitud previa y si la hay la cancela
		...
		$interes=$this->interesPlantilla($registro, $this->solicitud["plantilla"]);
		
		echo "<br>el numero de meses de interes es (".$interes.")*";


		$this->solicitud["observacion"]=$this->generarObservacion($registro, $this->solicitud["plantilla"]);
		...
		{			
			if(is_numeric($this->solicitud["cuota"]))
			{
				...
				for($i=1;$i<($this->solicitud["cuota"]+1);$i++)
				{
					...
					$dia=$registro[15+(3*($i-1))];
					$mes=$registro[16+(3*($i-1))];
					$anno=$registro[17+(3*($i-1))];
		...
		if(is_numeric($this->solicitud["solicitud"]))
		{
			...
			if($this->solicitud["plantilla"]==4  || $this->solicitud["plantilla"]==5)
			{
				$resultado=$this->guardarInteres($configuracion, $interes);
				if($resultado==false)
				{
					$this->errorProceso="No se pudo guardar el interes.";
					echo "<br>*11 No se pudo guardar el interes de ($interes)*  //funcion.class.php line 478";
					return false;
				}
				else{
					echo "<br>*11 Si se pudo guardar el interes de ($interes)*  //funcion.class.php line 478";
					return true;
				}		
	...
	function guardarExencion($configuracion, $exencion)
	{
		...
		switch($this->solicitud["plantilla"])
		{
			...
			case 3://Postgrados SMLV
				$idExencion = array(7 => 1, 8 => 10, 9 => 11, 10 => 12 , 11 => 5, 12 => 13, 13 => 14, 14 => 15);			
				break;
			case 4://Postgrados SMLV
				$idExencion = array(6 => 1, 7 => 2, 8 => 3, 9 => 4, 10 => 5 , 11 => 6, 12 => 7, 13 => 8, 14 => 9);		
				break;
			case 5://Postgrados SMLV
				$idExencion = array(6 => 1, 7 => 10, 8 => 11, 9 => 12 , 10 => 5, 11 => 13, 12 => 14, 13 => 15);			
				break;
		}
	...
	function guardarConcepto($configuracion, $concepto)
	{
		...
		echo "<br>primer concepto=".$cadena_sql;
	...
	function generarObservacion($registro, $tipo)
	{	
		switch($tipo)
		{
			case 4:
	...
	$observacion="RECIBO PERIODO ".$registro[$columnaInicial]."/".$registro[$columnaInicial+1];
	...
	function conceptoPlantilla($registro, $tipo)
	{
		...
		//se agrego un cuarto concepto para intereses pero este no se ve reflejado en la tabla de conceptos del recibo
		//por tanto no afecta la estructura
	...
	function interesPlantilla($registro, $tipo)
	{
		...
		switch($tipo)
		{
			case 4:
				$columnaInteres=5;
				$columnaInicioPeriodo=15;
		
			break;
			
			case 5:
				$columnaInteres=5;
				$columnaInicioPeriodo=14;			
		...
		if(isset($registro[$columnaInteres])){ //5 es posicion de la columna de interes
			
			$interes[0]=$registro[$columnaInteres];
			for($j=$columnaInicioPeriodo;$j<=($columnaInicioPeriodo+1);$j++){
				$interes[($j-$columnaInicioPeriodo)+1]=$registro[$j];
			}
	...
	function guardarInteres($configuracion,$interes)
	{
	
	include_once($configuracion["raiz_documento"].$configuracion["clases"]."/datosSolicitud.class.php");
	$datoSolicitud=new datosSolicitud();
	
	$this->solicitud["id_interes"]=$interes[0];
	$this->solicitud["anno_interes"]=$interes[1];
	$this->solicitud["periodo_interes"]=$interes[2];
	
		$cadena_sql=$this->sql->cadena_sql($configuracion, $this->acceso_db,"insertarInteres",$this->solicitud);	
		$resultado=$this->ejecutarSQL($configuracion, $this->acceso_db, $cadena_sql ," ");
		echo "<br>".$cadena_sql;
	return $resultado;
		...
		switch($this->solicitud["plantilla"])
		{
			...
			case 3:
				...
				for($i=1;$i<($this->solicitud["cuota"]+1);$i++)
				{
					...
					//Los porcentajes se encuentra en la columna 24,25,26
					if(isset($registro[23+$i]) && is_numeric($registro[23+$i]) && ($registro[23+$i]<100))
					{
						$this->solicitud["porcentajeCuota"]=$registro[23+$i];
					}
					else
					{
						//Valores por defecto
			...
			case 4: //extemporaneos pregrado
					...
					$this->solicitud["fechaExtraordinaria"]=$this->solicitud["fechaOrdinaria"];

					$this->solicitud["porcentajeCuota"]=0;
					$this->solicitud["cuotaGuardar"]=$this->solicitud["cuota"];
					$this->guardarCuota($configuracion, $this->solicitud);
			break;
			case 5://extemporaneos posgrado
