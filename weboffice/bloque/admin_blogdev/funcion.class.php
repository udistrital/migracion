...
class funciones_adminBlogdev extends funcionGeneral
{
	...
	$unUsuario=$this->verificarUsuario();
		if(is_array($unUsuario))
		{
			...
			//Valores a ingresar
			unset($valor);
			$valor[0]=$_REQUEST['editarAplicaciones'];
			$valor[1]=$_REQUEST['nom_aplicm'];
			$valor[2]=$_REQUEST['des_aplicm'];
			echo $valor[0];
			echo $valor[1]."<br>";
			echo $valor[2]."<br>";
			//echo $valor[3]."<br>";		
			$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"editaraplicaciones",$valor);
						...
						<table class="formulario" align="center">
							<tr  class="bloquecentralencabezado">
								<td colspan="2" align="center">
									<p><span class="texto_negrita">ADMINISTRACI&Oacute;N DEL FORMULARIO APLICACIONES</span></p>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								?>
								<input type='text' name='aplicacion' size='40' maxlength='50' tabindex='<? echo $tab++ ?>' >
								</td>
							</tr>
	...
	function mostrarRegistro($configuracion,$registro, $total, $opcion="",$valor)
	{
		switch($_REQUEST["accion"])
		{
			case "listaCompleta":
				...
				$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle, "totalAplic","");
				...
				else
				{
					...
					$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle, "insertadosAplic",$variable);
		...
		$menu=new navegacion();
		$variableNavegacion["pagina"]="admin_blogdev";
			...
			echo "<table class='formulario' align='center'>
					...
					<tr  class='bloquecentralencabezado'>
							<td colspan='4' align='center'>
								<p><span class='texto_negrita'>APLICACIONES </span></p>
							</td>
					</tr>
					...
					{	
						...
						$variable.="&opcion=editarAplicaciones";
	...
	function modificarAplicaciones($configuracion,$registro, $total, $opcion="",$variable)
	{
	$registroUsuario=$this->verificarUsuario();
	$contador=0;	
	$tab=0;
	$valor[1]=$_REQUEST['objeto'];
	$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle, "consultaAplicaciones",$valor);
	$resultado=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql, "busqueda");	
		if(is_array($resultado))
		{
			$aplicacioncod=$resultado[0][0];
			$aplicacionnom=$resultado[0][1];
			$aplicaciondes=$resultado[0][2];
			?><form enctype='multipart/form-data' method='POST' action='index.php' name='<? echo $this->formulario?>'>
				<table width="100%" align="center" border="0" cellpadding="10" cellspacing="0" >
					<tr>
						<td>	
							...
							<table class="formulario" align="center">
								<tr  class="bloquecentralencabezado">
									<td colspan="3" align="center">
										<p><span class="texto_negrita">MODIFICAR APLICACIONES</span></p>
									</td>
								</tr>
								<tr>
									<td colspan="3">
									...
									<td>
									<input maxlength="80" size="40" tabindex="<? echo $tab++ ?>" name="nom_aplicm" value="<? echo $aplicacionnom?>"><br>
									</td>
								</tr>
								</tr>
									<td>
									...
									<td>
									<input maxlength="80" size="40" tabindex="<? echo $tab++ ?>" name="des_aplicm" value="<? echo $aplicaciondes?>"><br>
									</td>
								</tr>
									...
									<td colspan="3">
										<table class="tablaBase">
											<tr>
												<td align="center">
													<input type='hidden' name='action' value='<? echo $this->formulario ?>'>
													<input type='hidden' name='aplicaciones' value='<? echo $valor[1] ?>'>
													<input type='hidden' name='editarAplicaciones' value='<? echo $valor[1] ?>'>
	...
	{
		$valor[1]=$_REQUEST['editarAplicaciones'];
		$cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle, "consultaAplicEditados",$valor);
		$resultado=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
	...
	function redireccionarInscripcion($configuracion, $opcion, $valor="")
	{
		...
		switch($opcion)
		{
			...
			case "registroeditado":
				$variable="pagina=admin_blogdev";
				$variable.="&opcion=exitoEditarAplic";
				$variable.="&editarAplicaciones=".$valor[0];
				break;	
