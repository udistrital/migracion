...
$cod_consul = "SELECT est_cod,
  					  ...
  					  v_acnot.cursada
				 FROM mntac.acest, mntac.ACMONTO, mntac.ACNOT, mntac.v_acnot, mntac.acnotobs, mntac.accra
				WHERE est_cod = v_acnot.not_est_cod
  				  ...
  				  AND acnot.not_ano = TO_NUMBER(DECODE(LENGTH(v_acnot.NOT_ASI_COD),1,SUBSTR(ultima,2,4),2,SUBSTR(ultima,3,4),3,SUBSTR(ultima,4,4),4,SUBSTR(ultima,5,4),5,SUBSTR(ultima,6,4),
  					  6,SUBSTR(ultima,7,4),7,SUBSTR(ultima,8,4),SUBSTR(ultima,9,4)))
  				  AND acnot.not_per  =   TO_NUMBER(DECODE(LENGTH(v_acnot.NOT_ASI_COD),1,SUBSTR(ultima,6,1),2,SUBSTR(ultima,7,1),3,SUBSTR(ultima,8,1),4,SUBSTR(ultima,9,1),5,SUBSTR(ultima,10,1),
