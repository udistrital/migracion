...
<?php
fu_cabezote("LISTAS DE CLASE Y CAPTURA DE NOTAS POSGRADO");
...
while(isset($consulta[$i][0]))
{
	if($consulta[$i][12]!='PREGRADO' && $consulta[$i][12]!='EXTENSION')
	{
		...
		<td align="left">
		<a href="doc_lis_clase.php?as='.$consulta[$i][8].'&gr='.$consulta[$i][10].'&C='.$consulta[$i][4].'" onMouseOver="link();return true;" onClick="link();return true;" title="Lista de clase">'.$consulta[$i][8].'</a></td>
		<td><a href="doc_dig_notasparPosgrado.php?A='.$consulta[$i][8].'&G='.$consulta[$i][10].'&C='.$consulta[$i][4].'" onMouseOver="link();return true;" title="Digitar notas">
		'.UTF8_DECODE($consulta[$i][9]).'</a></td>
