...
if(isset($_SESSION['carrera'])){
	...
	if($Nivel != 'POSGRADO')
	{ 
		die('<h3>No tiene asignaci&oacute;n acad&eacute;mica en posgrados.</h3>');
		exit;
	}
}
...
{
	echo "<script>location.replace(' ../err/err_sin_carga_posgrado.php')</script>";
	exit;
}
?>
  ...
  <tr>
	<td align="right" height="10" colspan="4" width="508"><b>Per&iacute;odo Acad&eacute;mico:</b></td>
	<td align="left" height="10" width="213"><? echo $anio.'-'.$peri; ?></td>
  </tr>
  <tr class="tr">
...
while(isset($consulta[$i][0]))
{
	...
	<a href="doc_lisclase_pos.php?as='.$consulta[$i][8].'&gr='.$consulta[$i][10].'&C='.$consulta[$i][4].'" onMouseOver="link();return true;" onClick="link();return true;" title="Lista de clase">'.$consulta[$i][8].'</a></td>
	<td width="25%" height="10">
	<a href="doc_dig_notas_pos.php?A='.$consulta[$i][8].'&G='.$consulta[$i][10].'&C='.$consulta[$i][4].'" onMouseOver="link();return true;" title="Digitar notas">'.$consulta[$i][9].'</a></td>
	<td width="5%" align="center" height="10">'.$consulta[$i][10].'</td>
	<td width="5%" align="center" height="10">'.$consulta[$i][11].'</td>
	<td width="20%" align="left" height="10"><span class="Estilo3">'.$consulta[$i][5].'</span></td></tr>';
$i++;
}
