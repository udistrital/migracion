...
/* Common Drupal methods definitons using in theme export */
...
function art_terms_worker($node) {
  ...
  if ($terms) {
    $links = array();
    ob_start();?><img class="art-metadata-icon" src="<?php echo get_full_path_to_theme(); ?>/images/PostTagIcon.png" width="18" height="18" alt=""/> <?php
	$output .= ob_get_clean();
    $output .= t('Tags: ');
    ...
    foreach ($links as $key => $link) {
	  ...
	  if ($class) {
        if (strpos ($class, "comment") !== FALSE) {
		  ob_start();?><img class="art-metadata-icon" src="<?php echo get_full_path_to_theme(); ?>/images/PostCommentsIcon.png" width="18" height="18" alt=""/> <?php
		  $output .= ob_get_clean();
        }
		else {
		  ob_start();?><img class="art-metadata-icon" src="<?php echo get_full_path_to_theme(); ?>/images/PostCategoryIcon.png" width="18" height="18" alt=""/> <?php
		  $output .= ob_get_clean();
        }
      }
...
function art_submitted_worker($submitted, $date, $name) {
  $output = '';
  ob_start();?><img class="art-metadata-icon" src="<?php echo get_full_path_to_theme(); ?>/images/PostDateIcon.png" width="17" height="18" alt=""/> <?php
  $output .= ob_get_clean();
  $output .= $date;
  $output .= '&nbsp;|&nbsp;';
  ob_start();?><img class="art-metadata-icon" src="<?php echo get_full_path_to_theme(); ?>/images/PostAuthorIcon.png" width="14" height="14" alt=""/> <?php
  $output .= ob_get_clean();
  $output .= $name;
  return $output;
}
