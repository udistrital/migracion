...
class WidgetHtml {
	...
	function cuadro_lista($atributos) {
		...
		if (isset($this->atributos["validar"])) {
			...
			$this->cadena_html.=" validate[" . $this->atributos["validar"] . "] ";
		}
		...
		/**
		 * Onchange atributo en cuadro_lista
		 * **/

		//          $this->cadena_html.="name='" . $nombre . "' size='" . $tamanno ;
			


		//         $this->cadena_html.= "' " . $mi_evento . " tabindex='" . $tab ."'";
		...
		if ($resultado) {
			...
			if ($seleccion == -1) {
				$this->cadena_html.="<option value='0' selected='true'>Seleccionar... </option>\n";
			}
	...
	private function definirEvento() {
		...
		switch ($evento) {
			...
			case 2 :
				if (isset($this->atributos["ajaxFunction"])) {
					$mi_evento = "onchange=\"" . $this->atributos["ajaxFunction"] . "\"";
				} else {
