...
// 	$pdf->Cell(180,6,"PBX.3239300 Ext.2600-2604",0,0,'C');
	...
	$Fecha_Retiroo=$funcion->calcularFecha3($fila['FECHA_RETIRO']);
	/*****************************************************/
	$mujer = "Señora";
	$hombre= "Señor";
	$tipoGenero = $fila['GENERO'];
	...
	if ($Fecha_Retiro ==NULL){
		...
		$pdf->Multicell(150,8,"Que ".$variable." ".trim($fila['PRIMER_NOMBRE'])." ".trim($fila['SEGUNDO_NOMBRE'])." ".trim($fila['PRIMER_APELLIDO'])." ".trim($fila['SEGUNDO_APELLIDO'])." ".$variable4." con ".$tipo_identificacion." ".$cedula.
				" de ".$lugar_expedicion.", estuvo vinculado a la institucion desde ".$Fecha_Ingreso." hasta ".$Fecha_Retiroo." donde se ".$desempeno." en el cargo de " .$fila['CARGO'].".",0,'J');
	}
	$pdf->Ln(10);
	$pdf->SetX(20);
	/*****************************************************/
    ...
    $pdf->Output($ruta."/documentos/basico_promediomensual.pdf",'F');    
    echo "<script language='javascript'>window.open('".$url."/basico_promediomensual.pdf','_self','');</script>";//para ver el archivo pdf generado
