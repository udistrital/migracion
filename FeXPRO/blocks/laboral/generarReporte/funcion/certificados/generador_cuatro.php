...
function Header()
	...
	function crearPDF($fila){
		...
		$mesada = money_format('%(#10n', $fila['MESADA']);
		...
		$tipoGenero = $fila['GENERO'];
		if ($tipoGenero == "M") {
			$variable2 = "pensionado";
		}
		else {
			$variable2 = "pensionada";
		}
		/*****************************************************/
		/*****************************************************/
		$tipoGenero = $fila['GENERO'];
		...
		//var_dump($fila['FECHA_RETIRO']);exit;
		$Fecha_Retiro=$funcion->calcularFecha($fila['FECHA_RETIRO']);
		/*****************************************************/
		$tipoGenero = $fila['GENERO'];
		if ($tipoGenero == "M") {
		...
		$pdf->Multicell(150,8,"Que ".$variable." ".trim($fila['PRIMER_NOMBRE'])." ".trim($fila['SEGUNDO_NOMBRE'])." ".trim($fila['PRIMER_APELLIDO'])." ".trim($fila['SEGUNDO_APELLIDO'])." ".$variable4." con ".$tipo_identificacion." ".$cedula.
				" de ".$lugar_expedicion.", se encuentra ".$variable2." por la institucion, desde ".$Fecha_Retiro.".",0,'J');
		$pdf->Ln(5);
		$pdf->SetX(20);
		$pdf->Multicell(150,8,"MESADA PENSIONAL......................................................................".$mesada."",0,'L');
		$pdf->Ln(5);
		$pdf->SetX(20);
		$pdf->Multicell(150,8,"Que ademas recibe dos mesadas adicionales una en el mes de JUNIO y la otra en el mes de DICIEMBRE.",0,'L');
		/*****************************************************/
		$year = "Año";
		/*****************************************************/
		$dia=date("l"); // Dia
		...
		$pdf->Output($ruta."/documentos/mesada_pensional.pdf",'F');
		echo "<script language='javascript'>window.open('".$url."/mesada_pensional.pdf','_self','');</script>";//para ver el archivo pdf generado
