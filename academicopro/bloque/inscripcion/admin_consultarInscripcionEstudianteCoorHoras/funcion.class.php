...
class funcion_adminConsultarInscripcionEstudianteCoorHoras extends funcionGeneral {
  ...
  function __construct($configuracion) {
    ...
    //$this->tema = $tema;
    $this->sql = new sql_adminConsultarInscripcionEstudianteCoorHoras();
    $this->log_us = new log();
    $this->formulario = "admin_consultarInscripcionEstudianteCoorHoras";
    $this->bloque = "inscripcion/admin_consultarInscripcionEstudianteCoorHoras";
...
<?
  ...
  function horarioEstudianteConsulta($resultadoInscritos, $datosEstudiante, $vista) {
    ...
    <table width="100%" border="0" align="center" cellpadding="5 px" cellspacing="1px" >
      <tr>
        <td>
      <?
      if (is_array($resultadoInscritos)) {//!is_null($resultadoInscritos)||
      ?>
          ...
          <tr>
            <td>
              <table class='contenidotabla sigma' width="100%">
                <thead class='sigma'>
                <th class='cuadro_plano sigma centrar' width="60">Cod.</th>
              ...
              ?>
                  ...
                  //Recorre el arreglo del resultado de los horarios
                                    for ($k = 0; $k < count($resultado_horarios); $k++) {
                                        ...
                                        if ($resultado_horarios[$k]['DIA'] == $i && $resultado_horarios[$k]['DIA'] == (isset($resultado_horarios[$k + 1]['DIA']) ? $resultado_horarios[$k + 1]['DIA'] : '') && (isset($resultado_horarios[$k + 1]['HORA']) ? $resultado_horarios[$k + 1]['HORA'] : '') == ($resultado_horarios[$k]['HORA'] + 1) && (isset($resultado_horarios[$k + 1]['ID_SALON']) ? $resultado_horarios[$k + 1]['ID_SALON'] : '') == ($resultado_horarios[$k]['ID_SALON'])) {
                                            ...
                                            $dia = "<strong>" . $resultado_horarios[$l]['HORA'] . "-" . ($resultado_horarios[$m]['HORA'] + 1) . "</strong><br>Sede: " . $resultado_horarios[$l]['COD_SEDE'] . "<br>Edificio: " . $resultado_horarios[$l]['EDIFICIO'] . "<br>Sal&oacute;n: ". $resultado_horarios[$l]['SALON'];
                                            echo $dia . "<br>";
                                            unset($dia);
                                        } elseif ($resultado_horarios[$k]['DIA'] == $i && $resultado_horarios[$k]['DIA'] != (isset($resultado_horarios[$k + 1]['DIA']) ? $resultado_horarios[$k + 1]['DIA'] : '')) {
                                            $dia = "<strong>" . $resultado_horarios[$k]['HORA'] . "-" . ($resultado_horarios[$k]['HORA'] + 1) . "</strong><br>Sede: " . $resultado_horarios[$k]['COD_SEDE'] . "<br>Edificio: " . $resultado_horarios[$k]['EDIFICIO'] . "<br>Sal&oacute;n: ". $resultado_horarios[$k]['SALON'];
                                            echo $dia . "<br>";
                                            unset($dia);
                                            $k++;
                ...
                ?></td><?
                  ...
                  $parametros.="&hor_alternativo=" . $resultadoInscritos[$j]['HOR_ALTERNATIVO'];
                ...
                ?>
                  ...
                  $variables = "pagina=admin_buscarGruposProyectoHoras";
                  $variables.="&opcion=cambiar";
                  $destino = "&parametro==";
                  $destino.="&destino=registro_cambiarGrupoEstudianteCoorHoras";
                  $destino.="&retorno=admin_consultarInscripcionEstudianteCoorHoras";
                ...
                ?>
                  ...
                  <?
                  $pagina = $this->configuracion["host"] . $this->configuracion["site"] . "/index.php?";
                  $variables = "pagina=registro_cancelarInscripcionEstudianteCoorHoras";
  ...
  function calcularInscritos($datosInscritos) {
      ...
      $contar_inscritos=$this->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql,"busqueda");
