                   ...
                   if(isset($registroEstudiante))
                       {
                           ...
                           $registro_permisos=$this->fechas->validar_fechas_estudiante_coordinador($configuracion,$registroEstudiante[0][0]);
...
<table class="sigma" align="center" width="70%">
	...
	function datosEstudiante($configuracion,$registro)
            ...
            <table class="contenidotabla centrar">
                    <tr>
                            <td>
                                <?echo "Nombre: <strong>".htmlentities(utf8_decode($registro[0][1]))."</strong>";?>
                            </td>
                            <td>
                    ...
                    <tr>
                        <td>
                            Plan de Estudios:
                                      <?echo "<strong>".htmlentities($registro[0][2])." - ".htmlentities(utf8_decode($registro[0][4]))."</strong><br>";?>
                        </td>
                        <td>
                            Proyecto Curricular:
...
<hr>
        ...
        function HorarioEstudianteConsulta($configuracion, $resultado_grupos,$registroEstudiante)
			...
			<tbody>
                ...
                for($j=0;$j<count($resultado_grupos);$j++){
                    ...
                    $cadena_sql=$this->sql->cadena_sql($configuracion,"clasificacionEspacio",$resultado_grupos[$j]['CODIGO']);//echo $cadena_sql;exit;
        ...
        function HorarioEstudianteInscripcion($configuracion, $resultado_grupos,$registroEstudiante,$planEstudioGeneral,$codProyecto)
            {
               ...
               $resultado_estado=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql,"busqueda" );
               $this->registroAgil($configuracion,$registroEstudiante);
                ?>
                    <table width="100%" border="0" align="center" cellpadding="5 px" cellspacing="1px" >
			<tbody>
                            <tr>
                ...
                for($j=0;$j<count($resultado_grupos);$j++){
                    ...
                    for($i=1; $i<8; $i++)
                    {
                        ...
                        for ($k=0;$k<count($resultado_horarios);$k++)
                        {
                            ...
                            if ($resultado_horarios[$k]['DIA']==$i && $resultado_horarios[$k]['DIA']==(isset($resultado_horarios[$k+1]['DIA'])?$resultado_horarios[$k+1]['DIA']:'') && (isset($resultado_horarios[$k+1]['SEDE'])?$resultado_horarios[$k+1]['SEDE']:'')==((isset($resultado_horarios[$k]['SEDE'])?$resultado_horarios[$k]['SEDE']:'')+1) && $resultado_horarios[$k+1]['SALON']==($resultado_horarios[$k]['SALON']))
                            {
                                $l=$k;
                                while ($resultado_horarios[$k]['DIA']==$i && $resultado_horarios[$k]['DIA']==(isset($resultado_horarios[$k+1]['DIA'])?$resultado_horarios[$k+1]['DIA']:'') && (isset($resultado_horarios[$k+1]['HORA'])?$resultado_horarios[$k+1]['HORA']:'')==($resultado_horarios[$k]['HORA']+1) && $resultado_horarios[$k+1]['SALON']==($resultado_horarios[$k]['SALON']))
                                {
				...
				?>
                                <a href="<?= $pagina.$variable ?>" on>
                                <img src="<?echo $configuracion['site'].$configuracion['grafico']?>/clean.png" width="30" height="30" border="0"><br><font size="1">Adicionar<br>Electivas Extrinsecas</font>
                                </a>
                                </td>
                            </tr>
                            </table>
        ...
        function registroAgil($configuracion,$registroEstudiante)
            {
                ?>
             <style type="text/css">
                #toolTipBox {
                 ...
                 <tr>
                     <td width="33%" class="cuadro_plano centrar">
                         <div style='width: 100%; height: 150px;border:0px solid #000000;'>
                             ...
                             <img src="<?echo $configuracion["site"].$configuracion["grafico"]."/viewrel.png"?>" style="cursor:pointer" border="0" onclick="xajax_buscarEspacios(document.getElementById('codEspacioAgil').value,<?echo $registroEstudiante[0][2]?>,<?echo $registroEstudiante[0][3]?>)"><br>
