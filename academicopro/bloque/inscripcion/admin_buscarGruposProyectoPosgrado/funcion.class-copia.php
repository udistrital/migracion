...
class funcion_adminBuscarGruposProyectoPosgrado extends funcionGeneral {
...
<?
    ...
    $this->enlaceBuscarGrupos($variables);
    ?>
  ...
  function presentarHorario($resultado_grupos,$estilo,$enlace,$estudiantes,$variables,$icono) {
    for ($j = 0; $j < count($resultado_grupos); $j++) {
      ...
      if ($permiso=='adicion')
        {
        if ($resultado_grupos[$j-1]['CARRERA']!=$resultado_grupos[$j]['CARRERA'])
          {
            ...
            <tr>
              <td>
                <table class='contenidotabla'>
                  <?$this->encabezadoHorario($estilo,$enlace,$estudiantes);
          }
        $variables['grupo'] = $resultado_grupos[$j]['GRUPO'];
        $variables['carrera'] = $resultado_grupos[$j]['CARRERA'];
        $this->horario($resultado_grupos[$j],$variables,$estilo,$icono,$estudiantes);
        if ($resultado_grupos[$j]['CARRERA'] != $resultado_grupos[$j + 1]['CARRERA'])
          {
          ?>
                </table>
              </td>
   ...
   */
  function enlaceBuscarGrupos($datos) {
    ?>
  <table width="100%" align="center" border="0" >
    <tr class="bloquelateralcuerpo">
      <td class="centrar" width="50%">
        ...
        $ruta = "pagina=admin_buscarGruposProyectoPosgrado";
   ...
   */
  function horario($gruposProyecto,$variables,$estilo,$icono,$estudiantes) {
    ?>
    <tr onmouseover="this.style.background='#F4F4EA'" onmouseout="this.style.background=''">
      ...
      if($estudiantes=="")
      {
        ...
        $this->enlaceCambioUno($estilo, $disponibles, $cruce, $gruposProyecto, $icono);
      }
      else
        {
        $cruce=$this->verificarCruceGrupo($variables);
        $numeroSeleccionados=$this->verificarSeleccionados();
        $numeroEstudiantesCruce=$this->numeroEstudiantesCruce($cruce);
        if($numeroEstudiantesCruce>0)
        {
          $estudiantesCruce=$this->estudiantesCruce($cruce);
//                    $cruce=$this->estudiantesSinCruce($cruce);
          }
//                  if ($cruce == FALSE) {
            $this->enlaceCambioSeleccion($estilo, $disponibles, $cruce, $gruposProyecto, $icono, $estudiantesCruce, $numeroEstudiantesCruce,$numeroSeleccionados);
//                  } else {
//                   }
        }
      ?>
   ...
   */
  function encabezadoHorario($estilo, $mensajeEnlace) {
    ?>
      <thead class='sigma'>
  ...
  function mostrarHorarioGrupo($datosGrupo, $estilo) {
    ...
    for ($i = 1; $i < 8; $i++) {
      ...
      for ($k = 0; $k < count($resultado_horarios); $k++) {
                      ...
                      if ($resultado_horarios[$k]['DIA'] == $i && $resultado_horarios[$k]['DIA'] == $resultado_horarios[$k + 1]['DIA'] && $resultado_horarios[$k + 1]['HORA'] == ($resultado_horarios[$k]['HORA'] + 1) && $resultado_horarios[$k + 1]['SALON'] == ($resultado_horarios[$k]['SALON'])) {
                        $l = $k;
                        while ($resultado_horarios[$k]['DIA'] == $i && $resultado_horarios[$k]['DIA'] == $resultado_horarios[$k + 1]['DIA'] && $resultado_horarios[$k + 1]['HORA'] == ($resultado_horarios[$k]['HORA'] + 1) && $resultado_horarios[$k + 1]['SALON'] == ($resultado_horarios[$k]['SALON'])) {
   ...
   * @param <string> $icono
   */
  function enlaceCambioUno($estilo, $disponibles, $cruce, $resultado_grupos, $icono) {
...
//            if($_REQUEST['codProyecto']!=$resultado_grupos['CARRERA'] && $disponibles <= '0')
            $grupo=$_REQUEST['grupo'];
            $_REQUEST['grupo']=$resultado_grupos['GRUPO'];
   ...
   */
  function enlaceCambioSeleccion($estilo,$disponibles,$cruce,$resultado_grupos,$icono,$estudiantesCruce,$numeroEstudiantesCruce,$seleccionados) {
    ?>
      ...
      <td class='<? echo $estilo ?>'<?  $this->consultarSeleccionadosCruce($seleccionados,$numeroEstudiantesCruce,$estudiantesCruce,$disponibles);?>>
        ...
        <form enctype='tipo:multipart/form-data,application/x-www-form-urlencoded,text/plain' method='POST' action='index.php' name='<? echo $_REQUEST['destino'] ?>'>
          ...
          $this->enlace($_REQUEST, $disponibles, $seleccionados, $numeroEstudiantesCruce,$icono)
          ?>
        </form>
   ...
   * @param <int> $numeroEstudiantesCruce
   */
  function enlace($resultado_grupos,$disponibles,$seleccionados,$numeroEstudiantesCruce,$icono) {
    $sobrecupo=$this->validacion->validarSobrecupo($resultado_grupos);
    //echo $sobrecupo."<br>".$disponibles."-".$seleccionados."+".$numeroEstudiantesCruce;
    if($sobrecupo!="ok" || ($disponibles-$seleccionados+$numeroEstudiantesCruce)<0)
      {
  ...
  function consultarSeleccionadosCruce($seleccionados,$numeroEstudiantesCruce,$estudiantesCruce,$disponibles) {
    ?>onmouseover="toolTip('<?echo "Seleccionados: ".$seleccionados."<br>Estudiantes con cruce: ".$numeroEstudiantesCruce."<br>";
   ...
   */
  function estudiantesSeleccionados($datosGrupo) {
...
$estudiantes = array();
$cruzados = array();
//        $_REQUEST['grupo']=$datosEstudiantes['grupo'];
foreach ($_REQUEST as $key => $value) {
  if (strstr($key, 'codEstudiante')) {
...
//        echo "para los siguientes estudiantes se presentan cruces de horario:<br>";
...
//        exit;
   ...
   */
  function numeroEstudiantesCruce($estudiantes) {
    $i=0;
    foreach ($estudiantes as $key => $value) {
      if ($value!="ok"){$i++;}
    }
   ...
   */
  function estudiantesCruce($estudiantes) {
    $i=0;
    foreach ($estudiantes as $key => $value) {
      if ($value!="ok")
        {
   ...
   */
  function estudiantesSinCruce($estudiantes) {
    $i=0;
    foreach ($estudiantes as $key => $value) {
      if ($value=="ok")
        {
