...
<table width="100%" border="0" align="center" cellpadding="1 px" cellspacing="0px" >
  ...
  function buscarGrupo($estudiantes="") {
    ...
    $variables = array('codEspacio' => $_REQUEST['codEspacio'],
                        ...
                        'codProyectoEstudiante' => (isset($_REQUEST['codProyectoEstudiante'])?$_REQUEST['codProyectoEstudiante']:$_REQUEST['codProyecto']),
  ...
  <table width="100%" align="center" border="0" >
    <tr class="bloquelateralcuerpo">
      <td class="centrar" width="50%">
        ...
        $ruta.="&codProyectoEstudiante=".(isset($_REQUEST['codProyectoEstudiante'])?$_REQUEST['codProyectoEstudiante']:$_REQUEST['codProyecto']);
  ...
  function enlaceCambioUno($estilo, $disponibles, $cruce, $resultado_grupos, $icono) {      
      ...
      <?
      if ($cruce == 'ok') {
            $existe_cadena_destino ='';
            $existe_cadena_destino = strpos($_REQUEST['destino'], "inscripcion/");
            if(!$existe_cadena_destino){
                $bloque_destino = "inscripcion/".$_REQUEST['destino'];
            }else{
                $bloque_destino=$_REQUEST['destino'];
            }
      ?>
          <form enctype='tipo:multipart/form-data,application/x-www-form-urlencoded,text/plain' method='POST' action='index.php' name='<? echo $_REQUEST['destino'] ?>'>
            ...
            <input type="hidden" name="grupoAnterior" value="<? echo (isset($_REQUEST['grupo'])?$_REQUEST['grupo']:'') ?>">
            <input type="hidden" name="id_grupoAnterior" value="<? echo (isset($_REQUEST['id_grupo'])?$_REQUEST['id_grupo']:'') ?>">
            <input type="hidden" name="opcion" value="inscribir">
            <input type="hidden" name="action" value="<? echo $bloque_destino;?>">
            <input type="hidden" name="retorno" value="<? echo $_REQUEST['retorno'] ?>">
            <input type="hidden" name="opcionRetorno" value="<? echo (isset($_REQUEST['opcionRetorno'])?$_REQUEST['opcionRetorno']:'') ?>">
    ...
    ?><script type="text/javascript" src="<? echo $this->configuracion["host"].$this->configuracion["site"].$this->configuracion["javascript"] ?>/overlib/overlib/overlib.js"><!-- overLIB (c) Erik Bosrup --></script>
      <td class='<? echo $estilo ?>'<?  $this->consultarSeleccionadosCruce($seleccionados,$numeroEstudiantesCruce,$estudiantesCruce,$disponibles);?>>
        ...
        <form enctype='tipo:multipart/form-data,application/x-www-form-urlencoded,text/plain' method='POST' action='index.php' name='<? echo $_REQUEST['destino'] ?>'>
          ...
          $datos['id_grupo']=$resultado_grupos['ID_GRUPO'];
          $this->enlace($datos, $disponibles, $seleccionados, $numeroEstudiantesCruce,$icono);
          unset ($datos);
          ?>
        </form>
