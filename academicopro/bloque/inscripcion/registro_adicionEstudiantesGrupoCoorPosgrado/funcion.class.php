 ...
 * @subpackage Admin
 */
class funcion_registroAdicionEstudiantesGrupoCoorPosgrado extends funcionGeneral {
  ...
  public $id_grupo;
    ...
    function __construct($configuracion) {
        ...
        $this->formulario = "registro_adicionEstudiantesGrupoCoorPosgrado";//nombre del BLOQUE que procesa el formulario
        $this->bloque = "inscripcion/registro_adicionEstudiantesGrupoCoorPosgrado";//nombre del BLOQUE que procesa el formulario
...
<?
    ...
    include_once($this->configuracion["raiz_documento"] . $this->configuracion["clases"] . "/validacion/validacionPosgradoCoordinador.class.php");
    ...
    foreach ($_REQUEST['codEstudiante'] as $key => $codigoEstudiante) {
            if(is_numeric($codigoEstudiante)){
                ...
                $this->validacionGrupo=new inscripcionPosgradoGrupoCoordinador();
            ...
            unset ($datosInscripcion);
                    ...
                    $arregloInforme[$a]['codigo']=(int)$codigoEstudiante;
                    $arregloInforme[$a]['nombre']='';
                    $arregloInforme[$a]['motivo']='Código de estudiante no valido';
                    $a++;
            }
      }
    ...
    function buscarDatosEstudiante($codigo) {
          ...
          if(is_array($arreglo_datosEstudiante))
            {
              if (is_null($arreglo_datosEstudiante[0]['PLANESTUDIOS']))
                {
                  $resultado[0]=array('CODIGO'=>$codigo);
                  echo "El estudiante con c&oacute;digo ".$codigo." no registra plan de estudios. Reg&iacute;strelo por Datos B&aacute;sicos del estudiante.";exit;
                }
    ...
    function buscarCodigoEstudiante($codigo) {
          ...
          $cadena_sql = $this->sql->cadena_sql("buscarCodigoEstudiante", $codigo);
          $arreglo_datosEstudiante = $this->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
    ...
    function crearArregloDatosInscripcion($datosEstudiante,$datosEspacio){
        $arregloDatosInscripcion=array(   'codEstudiante'=>$datosEstudiante['CODIGO'],
                                          ...
                                          'codEspacio'=>  $this->codEspacio,
                                          'creditos'=> (isset($datosEspacio[0]['CREDITOS'])?$datosEspacio[0]['CREDITOS']:''),
                                          'htd'=> (isset($datosEspacio[0]['HTD'])?$datosEspacio[0]['HTD']:''),
                                          'htc'=> (isset($datosEspacio[0]['HTC'])?$datosEspacio[0]['HTC']:''),
                                          'hta'=> (isset($datosEspacio[0]['HTA'])?$datosEspacio[0]['HTA']:''),
                                          ...
                                          'registro'=>$this->ano.'-'.$this->periodo.','.$this->codEspacio.',0,'.$this->id_grupo.','.$this->grupo.','.$datosEstudiante['PLANESTUDIOS'].','.$this->codProyecto,
