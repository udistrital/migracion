...
?><a name="1"></a><?
    ...
    $cadena_idGrupos=$this->procedimientos->crearCadenaIdGrupos($resultado_grupos);
    $resultado_horarios=$this->consultarHorarios($cadena_idGrupos);
...
<BR><BR><a class="scroll" href="#2">Ver grupos de otros proyectos</a><img src="<? echo $this->configuracion['site'] . $this->configuracion['grafico'] ?>/down2.png" width="15" height="10" border="0">
 <div class="tablaEspacios" align="left"><div class="nombreEspacio" ><b><? echo $_REQUEST['codEspacio'] . " - " . $_REQUEST['nombreEspacio']; ?></b></div>
    ...
    if(isset($gruposProyecto)&&is_array($gruposProyecto))
    {
        ...
        for ($j = 0; $j < $totalGruposProyecto; $j++)
        {
          ...
          if ($permiso=='adicion')
            ...
            <div width="100%" border="0" align="center" cellpadding="2px" cellspacing="1px" ><div class="nombrecarrera" ><b><? echo "PROYECTO CURRICULAR: " . $carrera ?> </div>
              ...
              if (is_array($resultado_horarios))
              {
                  foreach ($resultado_horarios as $horarios) {
                      if($horarios['ID_GRUPO']==$gruposProyecto[$j]['ID_GRUPO'])
                      {
            ...
            if ((isset($gruposProyecto[$j-1]['ID_GRUPO'])?$gruposProyecto[$j-1]['ID_GRUPO']:'') != $gruposProyecto[$j]['ID_GRUPO'])
            {
                $variables['id_grupo'] = $gruposProyecto[$j]['ID_GRUPO'];
          ...
          if ($permiso=='adicion')
            ...
            if ((isset($gruposOtros[$j-1]['CARRERA'])?$gruposOtros[$j-1]['CARRERA']:'')!=$gruposOtros[$j]['CARRERA'])
             ...
             <div width="100%" border="0" align="center" cellpadding="2px" cellspacing="1px" ><div class="nombrecarrera" ><b><? echo "PROYECTO CURRICULAR: " . $carrera ?> </div>
              ...
              {
                  foreach ($resultado_horarios as $horarios) {
                      if($horarios['ID_GRUPO']==$gruposOtros[$j]['ID_GRUPO'])
                      {
                          $horarioGrupo[]=$horarios;
                      }
                  }
            ...
            if ((isset($gruposOtros[$j-1]['ID_GRUPO'])?$gruposOtros[$j-1]['ID_GRUPO']:'') != $gruposOtros[$j]['ID_GRUPO'])
            {
                $variables['id_grupo'] = $gruposOtros[$j]['ID_GRUPO'];
   ...
   * @param <string> $icono
   */
  function crearEnlaceCambioUno($estilo, $disponibles, $cruce, $resultado_grupos, $icono,$horario) {
              ...
              elseif($disponibles>0)
                {
                    ...
                    $variables.="&action=inscripcion/".$_REQUEST['destino'];
                    ...
                    $parametros.="&nivel=" . (isset($_REQUEST['nivel'])?$_REQUEST['nivel']:'');
                    $parametros.="&creditos=" . (isset($_REQUEST['creditos'])?$_REQUEST['creditos']:'');
                    $parametros.="&id_grupo=" . $resultado_grupos['ID_GRUPO'];
                    $parametros.="&hor_alternativo=" . $horario['HOR_ALTERNATIVA'];
                    $parametros.="&cupo=" . $resultado_grupos['CUPO'];
                    $parametros.="&carrera=" . $resultado_grupos['CARRERA'];
                    $parametros.="&grupoAnterior=".$_REQUEST['grupo'];
                    $parametros.="&id_grupoAnterior=".$_REQUEST['id_grupo'];
