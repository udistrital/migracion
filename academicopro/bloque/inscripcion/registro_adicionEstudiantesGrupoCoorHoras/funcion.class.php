...
class funcion_registroAdicionEstudiantesGrupoCoorHoras extends funcionGeneral {
  ...
  /**
    ...
    function __construct($configuracion) {
        ...
        $this->formulario = "registro_adicionEstudiantesGrupoCoorHoras";//nombre de la página que procesa el formulario
        $this->bloque = "inscripcion/registro_adicionEstudiantesGrupoCoorHoras";//nombre del BLOQUE que procesa el formulario
          ...
          <caption class="sigma centrar">
            ...
            <tr class="sigma derecha">
              <td width="18%">
                ...
                <input type="hidden" name="id_grupo" value="<? echo $_REQUEST['id_grupo'] ?>">
              </td>
              <td colspan="2">
...
<?
          ...
          if($resultado_validacion=='ok')
            {
            ...
            }elseif($resultado_validacion=='4')
                {
                    ...
                    $this->realizarInscripcion($datosInscripcion);
                }//si el espacio no pertenece al plan verifica si esta en las tablas de homologacion
                elseif(stristr($resultado_validacion,'No pertenece al plan')!==FALSE){
                        $espaciosHomologos=$this->consultarEspaciosHomologosCreditos($datosInscripcion);
                        //si es homologo hace la validacion para la inscripcion
                        if (is_array($espaciosHomologos)&&!empty($espaciosHomologos[0][0])){
                            //verifica por cada espacio registrado
                            foreach ($espaciosHomologos as $key => $espacio)
                            {
                                $datosInscripcionHomologo=$datosInscripcion;
                                $datosInscripcionHomologo['codEspacio']=$espacio['ASI_COD_ANTERIOR'];
                                $datosInscripcionHomologo['codProyecto']=$datosInscripcionHomologo['codProyectoEstudiante'];
                                $datosInscripcionHomologo['planEstudio']=$datosInscripcionHomologo['planEstudioEstudiante'];
                                //valida el cruce con los datos del espacio homologo a inscribir en el proyecto del coordinador
                                $validaciones=array(0=>'validarCruceHorario');
                                $resultado_validacion=$this->validacionGrupo->realizarValidacion($validaciones,$datosInscripcion);
                                if($resultado_validacion=='ok')
                                {
                                    //valida la inscripcion con los doatos del espacio en el plan del estudiante
                                    $validaciones=array(0=>'validarEspacioPlanCreditos',
                                                        1=>'validarCreditosInscritos',
                                                        2=>'validarCreditosPorClasificacion',
                                                        3=>'validarRequisitosCreditos');
                                    $resultado_validacion=$this->validacionGrupo->realizarValidacion($validaciones,$datosInscripcionHomologo);
                                }else{
                                        $resultado_validacion=$this->validacionGrupo->formatearResultadoValidacion($resultado_validacion);
                                        break;
                                    }
                                if($resultado_validacion=='ok')
                                {
                                    $codEspacio=$this->codEspacio;
                                    $this->codEspacio=$datosInscripcionHomologo['codEspacio'];
                                    $codProyecto=$this->codProyecto;
                                    $this->codProyecto=$datosInscripcionHomologo['codProyectoEstudiante'];
                                    $planEstudio=$this->planEstudio;
                                    $this->planEstudio=$datosInscripcionHomologo['planEstudioEstudiante'];
                                    $datos_EspacioAcademico=$this->buscarDatosEspacio();
                                    $this->codEspacio=$codEspacio;
                                    $this->codProyecto=$codProyecto;
                                    $this->planEstudio=$planEstudio;
                                    $datosInscripcion['htd']=$datos_EspacioAcademico[0]['HTD'];
                                    $datosInscripcion['htc']=$datos_EspacioAcademico[0]['HTC'];
                                    $datosInscripcion['hta']=$datos_EspacioAcademico[0]['HTA'];
                                    $datosInscripcion['cea']=$datos_EspacioAcademico[0]['CEA'];
                                    $this->realizarInscripcion($datosInscripcion);
                                    break;
                                }else{
                                    $resultado_validacion=$this->validacionGrupo->formatearResultadoValidacion($resultado_validacion);
    ...
    /**
     * Esta funciónpermite consultar los espacios homologos del proyecto para estudaintes de creditos
     * @param type $datosInscripcion
     * @return type
     */
    function consultarEspaciosHomologosCreditos($datosInscripcion)
    {
          //var_dump($datosInscripcion);exit;
          $variablesDatosEstudiante = array('codProyectoEstudiante'=>$datosInscripcion['codProyectoEstudiante'],
                                            'codEspacio'=>$datosInscripcion['codEspacio']);
          $cadena_sql = $this->sql->cadena_sql("consultarEspaciosHomologos", $variablesDatosEstudiante);//echo $cadena_sql;exit;
          $arreglo_espaciosHomologos = $this->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
          return $arreglo_espaciosHomologos;
    ...
    function crearArregloDatosInscripcion($datosEstudiante,$datosEspacio){
        $arregloDatosInscripcion=array(   'codEstudiante'=>$datosEstudiante['CODIGO'],
                                          ...
                                          'registro'=>$this->ano.'-'.$this->periodo.','.$this->codEspacio.',0,'.$this->id_grupo.','.$datosEstudiante['PLANESTUDIOS'].','.$this->codProyecto,
