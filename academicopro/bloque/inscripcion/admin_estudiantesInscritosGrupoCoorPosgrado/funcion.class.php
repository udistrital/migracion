<?php
/**
 * Funcion admin_estudiantesInscritosGrupoCoordinadorPosgrado
 *
 * Esta clase se encarga de crear la logica y mostrar la interfaz de usuario
 *
 * @package InscripcionCoordinadorPosgradoPorGrupo
 * @subpackage Consulta
...
/**
 * Clase funcion_admin_estudiantesInscritosGrupoCoordinadorPosgrado
 *
 * Clase que contiene los métodos que ejecutan tareas y crean los formularios de la pagina del bloque.
 * @package InscripcionCoordinadorPosgradoPorGrupo
 * @subpackage Consulta
 */
class funcion_estudiantesInscritosGrupoCoordinadorPosgrado extends funcionGeneral
{
  ...
  /**
	...
	function __construct($configuracion)
	    ...
	    $this->sql=new sql_estudiantesInscritosGrupoCoordinadorPosgrado($configuracion);
            
            $this->formulario="admin_estudiantesInscritosGrupoCoorPosgrado";
            $this->bloque="inscripcion/admin_estudiantesInscritosGrupoCoorPosgrado";
        ...
        function armarVistaGrupo() {
          ...
          <div style="margin: 5px">
              <table class="sigma contenidotabla">
                ...
                $this->mostrarDatosEstudiante($resultadoDatosEstudiante[0],$numero,$proceso);
        ...
        function mostrarDatosProyecto($proyecto) {
              ...
              <table class="sigma_borde centrar" width="100%">
                              <caption class="sigma centrar">
                                <?echo $proyecto['CODIGO'].' - '.$proyecto['NOMBRE'].' - Plan de Estudios '.$this->planEstudio?>
                              </caption>
              </table>
...
?>  
        ...
        function consultarHorario() {
          ...
          if(isset($_REQUEST['id_grupo'], $this->codEspacio)){
          ...
          if (is_array($horario)) {
              for ($i = 1; $i < 8; $i++) {
                  ...
                  for ($k = 0; $k < count($horario); $k++) {
                                        ...
                                        if ($horario[$k]['DIA'] == $i && $horario[$k]['DIA'] == (isset($horario[$k + 1]['DIA']) ? $horario[$k + 1]['DIA'] : '') && (isset($horario[$k + 1]['HORA']) ? $horario[$k + 1]['HORA'] : '') == ($horario[$k]['HORA'] + 1) && (isset($horario[$k + 1]['ID_SALON']) ? $horario[$k + 1]['ID_SALON'] : '') == ($horario[$k]['ID_SALON'])) {
                                            $l = $k;
                                            while ($horario[$k]['DIA'] == $i && $horario[$k]['DIA'] == (isset($horario[$k + 1]['DIA']) ? $horario[$k + 1]['DIA'] : '') && (isset($horario[$k + 1]['HORA']) ? $horario[$k + 1]['HORA'] : '') == ($horario[$k]['HORA'] + 1) && (isset($horario[$k + 1]['ID_SALON']) ? $horario[$k + 1]['ID_SALON'] : '') == ($horario[$k]['ID_SALON'])) {

                                                $m = $k;
                                                $m++;
                                                $k++;
                                            }
                                            $dia = "<strong>" . $horario[$l]['HORA'] . "-" . ($horario[$m]['HORA'] + 1) . "</strong><br>Sede: " . (isset($horario[$l]['SEDE'])?$horario[$l]['SEDE']:'') . "<br>Edificio: " . (isset($horario[$l]['EDIFICIO'])?$horario[$l]['EDIFICIO']:'') . "<br>Sal&oacute;n:" . (isset($horario[$l]['SALON'])?$horario[$l]['SALON']:'');
                                            echo $dia . "<br>";
                                            unset($dia);
                                        } elseif ($horario[$k]['DIA'] == $i && $horario[$k]['DIA'] != (isset($horario[$k + 1]['DIA']) ? $horario[$k + 1]['DIA'] : '')) {
                                            $dia = "<strong>" . $horario[$k]['HORA'] . "-" . ($horario[$k]['HORA'] + 1) . "</strong><br>Sede: " . $horario[$k]['SEDE'] . "<br>Edificio: " . $horario[$k]['EDIFICIO'] . "<br>Sal&oacute;n:" . $horario[$k]['ID_SALON'] . " " . $horario[$k]['SALON'];
                                            echo $dia . "<br>";
                                            unset($dia);
                                            $k++;
                                        } elseif ($horario[$k]['DIA'] == $i && $horario[$k]['DIA'] == (isset($horario[$k + 1]['DIA']) ? $horario[$k + 1]['DIA'] : '') && (isset($horario[$k + 1]['ID_SALON']) ? $horario[$k + 1]['ID_SALON'] : '') != ($horario[$k]['ID_SALON'])) {
                                            $dia = "<strong>" . $horario[$k]['HORA'] . "-" . ($horario[$k]['HORA'] + 1) . "</strong><br>Sede: " . $horario[$k]['SEDE'] . "<br>Edificio: " . $horario[$k]['EDIFICIO'] . "<br>Sal&oacute;n:" . $horario[$k]['ID_SALON'] . " " . $horario[$k]['SALON'];
                                            echo $dia . "<br>";
                                            unset($dia);
                                        } elseif ($horario[$k]['DIA'] != $i) {                        }
...
//        }
        ...
        function mostrarDatosEstudiante($estudiante, $numero, $proceso){
          ?>
              ...
              <?if($proceso=='adicion'||$proceso=='cancelacion')
                  {
                    if($estudiante['PLAN']==$this->planEstudio && $estudiante['PROYECTO']==$this->codProyecto){?>
                ...
                ?>
              <td align="center"><?echo $estudiante['CODIGO']?></td>
              <td><?echo htmlentities($estudiante['NOMBRE'])?></td>
              <td align="center"><?echo $estudiante['PROYECTOABREV']?></td>
              <td align="center"><?echo $estudiante['ESTADO']?></td>
          <?
        }
        ...
        function mostrarEnlaces($datosEstudiante,$datosEspacio,$tipo){
          ...
          switch ($tipo) {
            ...
            if($datosEstudiante['PLAN']==$this->planEstudio && $datosEstudiante['PROYECTO']==$this->codProyecto){
          foreach ($enlace as $key => $value) {
            ?><td align="center"><a href="<? echo $this->$key($datosEstudiante, $datosEspacio) ?>" ><img src="<? echo $this->configuracion["site"] . $this->configuracion["grafico"] . "/".$value.".png" ?>" border="0" width="20" height="20"></a></td><?
              ...
              {
              if ($col>0){
              ?><td align="center" colspan="<?echo $col;?>">No pertenece al plan <?echo $this->planEstudio?></td><?
              }else
              {}
        ...
        function enlaceCambiar($datosEstudiante,$datosEspacio) {
          $pagina = $this->configuracion["host"] . $this->configuracion["site"] . "/index.php?";
          $variable="pagina=admin_buscarGruposProyectoPosgrado";
          ...
          $variable.="&parametro==";
          $variable.="&destino=registro_cambiarGrupoEstudianteCoorPosgrado";
          $variable.="&retorno=admin_estudiantesInscritosGrupoCoorPosgrado";
              ...
              ?><table>
                  <tr>
                    ...
                    <td><input type="hidden" name="opcion" value="procesar">
                      ...
                      <input type="hidden" name="destino" value="registro_cambiarGrupoEstudianteGrupoCoorPosgrado">
                      <input type="hidden" name="retorno" value="admin_estudiantesInscritosGrupoCoorPosgrado">
