...
class funciones_registroElectivasCoordinador extends funcionGeneral {     	//Crea un objeto tema y un objeto SQL.
...
//        $this->administrar->administrarModuloSGA($configuracion, '4');
        ...
        //Datos de sesion
        $this->formulario="registroElectivasCoordinador";
    ...
    function consultarEspaciosPermitidos($configuracion) {
        ...
        $cadena_sql_planEstudio=$this->sql->cadena_sql($configuracion,$this->accesoGestion,"electivas_extrinsecas", $permitidos);//echo $cadena_sql_planEstudio;exit;
        $resultado_planEstudio=$this->ejecutarSQL($configuracion, $this->accesoGestion, $cadena_sql_planEstudio,"busqueda" );
        ...
        for($i=0;$i<count($resultado_planEstudio);$i++)
            {
                $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"buscar_cargado", $resultado_planEstudio[$i][0]);//echo $cadena_sql_planEstudio;exit;
...
<table class='contenidotabla' align='center' width='80%' cellpadding='2' cellspacing='2'>
    <thead class='cuadro_color centrar'>
    <td>Carrera Ofrece</td><td>Codigo Espacio</td><td>Nombre Espacio</td><td>Clasificaci&oacute;n</td><td>Nro Cr&eacute;ditos</td><td>Adicionar</td>
    </thead>
            <?
                    for($i=0;$i<count($cargados);$i++) {
                        $band = '0';
                        //echo "<br>".$cargados[$i][0]."-".$cargados[$i][1]."-".$cargados[$i][2];
                        $cadena_sql_espacio=$this->sql->cadena_sql($configuracion,$this->accesoGestion,"nombre_espacio", $cargados[$i][0]);//echo "<br>".$cadena_sql_espacio;//exit;
                        $resultado_espacio=$this->ejecutarSQL($configuracion, $this->accesoGestion, $cadena_sql_espacio,"busqueda" );

                        $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"carrera_ofrece", $cargados[$i]);//echo "<br>".$cadena_sql;exit;
                            ...
                            if ($band == '0') {
                                ?> <tr>
                                    <td class='cuadro_plano centrar'><? echo $resultado_carrera[0][0]?></td><!--Carrera que la ofrece-->
                                        <td class='cuadro_plano centrar'><? echo $cargados[$i][0]?></td><!--Codigo del espacio -->
            ...
            <form enctype='tipo:multipart/form-data,application/x-www-form-urlencoded,text/plain' method='POST' action='index.php' name='<?echo $this->formulario?>'>
                <input type="hidden" name="espacio" value="<?echo $cargados[$i][0]?>">
