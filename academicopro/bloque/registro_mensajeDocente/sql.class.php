 ...
 * @subpackage Consulta
 */
class sql_registroMensajeDocente extends sql
{
   ...
   function cadena_sql($tipo,$variable="")
	{
	 ...
	 switch($tipo)
	 {
              ...
              //Oracle
              case 'buscarEstudianteDestinatario':

                $cadena_sql="SELECT";
                $cadena_sql.=" eco_est_cod CODIGO,";
                $cadena_sql.=" est_nombre NOMBRE_ESTUDIANTE,";
                $cadena_sql.=" eco_doc_nro_ident IDENTIFICACION_DOCENTE,";
                $cadena_sql.=" eco_cra_cod CARRERA,";
                $cadena_sql.=" eco_fecha FECHA,";
                $cadena_sql.=" eco_estado ESTADO,";
                $cadena_sql.=" est_ind_cred TIPO";
                $cadena_sql.=" FROM acestudianteconsejero";
                $cadena_sql.=" INNER JOIN acest on eco_est_cod= est_cod";
                $cadena_sql.=" WHERE eco_doc_nro_ident=".$variable['codUsuario'];
                $cadena_sql.=" AND eco_estado LIKE '%A%'";

                break;

              //Oracle
              case 'buscarCoordinadorDestinatario':

                $cadena_sql="SELECT";
                $cadena_sql.=" cra_emp_nro_iden CODIGO,";
                $cadena_sql.=" doc_apellido APELLIDO,";
                $cadena_sql.=" doc_nombre NOMBRE,";   
                $cadena_sql.=" 28 TIPO";
                $cadena_sql.=" FROM accra";
                $cadena_sql.=" INNER JOIN acdocente ON cra_emp_nro_iden=doc_nro_iden";
