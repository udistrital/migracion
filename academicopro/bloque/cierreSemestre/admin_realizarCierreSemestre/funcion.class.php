    ...
    <body><p></p>
        <div class="ProgressBar">
          ...
          <div id="getProgressBarFillNotas"></div>
            ...
            $variable.="&action=cierreSemestre/admin_cierreSemestre";
        ...
        function actualizarNota($registroNota,$acumulado) {
             $datos=array(
                        ...
                        'codEstudiante'=>$registroNota['COD_ESTUDIANTE'],
                        'codProyecto'=>$registroNota['COD_PROYECTO'],
                        'codEspacio'=>$registroNota['COD_ESPACIO'],
                        'acumulado'=>$acumulado,
                        'nota'=>round($acumulado, 0, PHP_ROUND_HALF_UP)
                        );
        ...
        function cargarNotas($codProyecto) {
            ...
            //$totalHuerfanos=0;
                ...
                if($numeroEstudiantes>0){
                    $this->notasInscripcionesProyecto=$this->consultarNotasInscripcionesProyecto($codProyecto);
                    //$numeroEstudiante=0;
                    $a=1;
                    ...
                    foreach ($estudiantes as $estudiante) {
                        ...
                        $notasInscripciones=$this->buscarNotasInscripcionesEstudiante($estudiante['CODIGO']);
     ...
     */
        ...
        foreach ($homologaciones as $homologacion) {
            $datosRegistro = array('NOT_CRA_COD'=>$datos_estudiante['COD_PROYECTO'],
                                            ...
                                            'NOT_NOTA'=>(isset($homologacion['NOTA'])?$homologacion['NOTA']:''),
        ...
        function ejecutarUnoAUno($datos_estudiante,$homologaciones){
            foreach ($homologaciones as $homologacion)
            {
                $datosRegistro = array('NOT_CRA_COD'=>$datos_estudiante['COD_PROYECTO'],
                                                ...
                                                'NOT_NOTA'=>(isset($homologacion['NOTA']['NOTA'])?$homologacion['NOTA']['NOTA']:''),
            ...
            foreach ($homologaciones as $homologacion)
            {
                for($i=1;$i<3;$i++)
                {
                    $datosRegistro = array('NOT_CRA_COD'=>$datos_estudiante['COD_PROYECTO'],
                                                    ...
                                                    'NOT_NOTA'=>(isset($homologacion['NOTA_'.$i]['NOTA'])?$homologacion['NOTA_'.$i]['NOTA']:''),
   ...
   */
     ...
     */
        ...
        */
        ...
        /**
        * Funcion que consulta las notas del proyecto que existen en las inscripciones
        * @param <int> $cod_estudiante
        */
        function consultarNotasInscripcionesProyecto($cod_proyecto){
          $datos= array('cod_proyecto'=>$cod_proyecto,
                        'ano'=>$this->ano,
                        'periodo'=>$this->periodo
                  );
         $cadena_sql = $this->sql->cadena_sql("consultarNotasInscripcionesProyecto",$datos);
         $resultado = $this->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
         return $resultado;
       }    
