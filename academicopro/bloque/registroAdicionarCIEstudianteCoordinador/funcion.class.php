...
class funciones_registroAdicionarCIEstudianteCoordinador extends funcionGeneral {     	//Crea un objeto tema y un objeto SQL.
...
//        $this->administrar->administrarModuloSGA($configuracion, '4');
        ...
        //Datos de sesion
        $this->formulario="registroAdicionarCIEstudianteCoordinador";
    ...
    function consultarEspaciosPermitidos($configuracion)
    {
        ...
        $numeroCreditosRestantes=('4'-$_REQUEST['creditosInscritos']);
...
<table class='contenidotabla' align='center' width='80%' cellpadding='2' cellspacing='2'>
    <thead class='cuadro_color centrar'>
    <td>Codigo Espacio</td><td>Nombre Espacio</td><td>Clasificaci&oacute;n</td><td>Nro Cr&eacute;ditos</td><td>Adicionar</td>
    </thead>
            <?
            $nivelAnterior=0;
                    for($i=0;$i<count($resultado_planEstudio);$i++) {
                        ...
                        if(is_array($resultado_espacio)){
                            ...
                            if ($resultado_otroRequisito[0][0]=='2') {
                                ...
                                $variablesHorario=array($resultado_planEstudio[$i][0],$_REQUEST['codProyecto'],$ano[0],$ano[1]);

                                $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"horario_espacios_academicos", $variablesHorario);//echo $cadena_sql."<br>";//exit;
                                $resultado_horario=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql,"busqueda" );
                                ...
                                if(trim($resultado_planEstudio[$i][2])!=$nivelAnterior)
                                    {
                                        ...
                                        <tr class="cuadro_color centrar">
                                            <td colspan="6"><font size="2"> NIVEL <? echo $resultado_planEstudio[$i][2]?></font></td>
                                        </tr>
                                        <?
                                    }
                                ?> <tr>
                                    ...
                                    if($resultado_espacio[0][1]<=$numeroCreditosRestantes) {
                                        ...
                                        ?>
                                            ...
                                            <td class='cuadro_plano centrar'>
                                            No puede adicionar<br>mas de 4 cr&eacute;ditos
                                            </td>
                                        <?
                                        }
                                        ?>
...
<table class="cuadro_color centrar" width="100%">
    ...
    function buscarGrupo($configuracion)
    {
        ...
        $variables=array($espacio,$resultado_craCoordinador[0][1],$planEstudio);
                            ...
                            <table class='contenidotabla'>
                                ...
                                <td class='cuadro_plano centrar' >Adicionar</td>
                                            ...
                                            for($j=0;$j<count($resultado_grupos);$j++) {
                                                ...
                                                $cadena_sql_horarios=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"horario_grupos", $variables);//echo $cadena_sql_horarios;
...
//var_dump($_REQUEST);exit;
        ...
        $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoOracle,"espacio_planEstudio", $var_espacio);//echo $cadena_sql;exit;
        ...
        if ($resultado_EspacioOracle =='' and $resultado_buscarEspacioMysql == '') {
            ...
            if($resultado_cupo_inscritos[0][0]<=$resultado_cupo_grupo[0][0] || $this->nivel=28) {
                ...
                $creditos=($_REQUEST['creditosInscritos']+$_REQUEST['creditos']);
                //echo $creditos;
                if ($creditos<='4') {
                    ...
                    $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoGestion,"nombre_espacio", $_REQUEST['espacio']);
                    $resultado_nombreEspacio=$this->ejecutarSQL($configuracion, $this->accesoGestion, $cadena_sql,"busqueda" );

                    echo "<script>alert ('No se puede inscribir el Espacio Académico [".$_REQUEST['espacio']." - ".$resultado_nombreEspacio[0][0]."]. El número de créditos inscritos para el estudiante es mayor a 4');</script>";
                    $pagina=$configuracion["host"].$configuracion["site"]."/index.php?";
                    $variable="pagina=adminConsultarCIEstudianteCoordinador";
                    $variable.="&opcion=mostrarConsulta";
                    $variable.="&codEstudiante=".$_REQUEST["codEstudiante"];
    ...
    function verificarCancelacion($configuracion)
    {
            ...
            $cadena_sql=$this->sql->cadena_sql($configuracion,$this->accesoGestion,"estudianteCancelo", $variablesCancelado);
