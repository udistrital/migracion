...
class sql_registroAdicionarCIEstudianteCoordinador extends sql {
    function cadena_sql($configuracion,$conexion, $opcion,$variable="") {
        ...
        switch($opcion) {
            ...
            case 'horario_espacios_academicos':

                $cadena_sql="select * from achorario ";
                $cadena_sql.="inner join accurso on achorario.hor_nro=accurso.cur_nro and achorario.hor_asi_cod=accurso.cur_asi_cod ";
                $cadena_sql.=" where hor_asi_cod=".$variable[0];
                $cadena_sql.=" and cur_cra_cod=".$variable[1];
                $cadena_sql.=" and hor_ape_ano=".$variable[2];
                $cadena_sql.=" and hor_ape_per=".$variable[3];
            ...
            case 'espacios_plan_estudio':
                ...
                $cadena_sql.=" and not_nota>=30) and pen_asi_cod not in";
            ...
            case 'adicionar_espacio_oracle':
                ...
                $cadena_sql.="'0',";
                $cadena_sql.="(SELECT APE_ANO FROM ACASPERI WHERE APE_ESTADO= 'V'),";
                $cadena_sql.="(SELECT APE_PER FROM ACASPERI WHERE APE_ESTADO= 'V'),";
                $cadena_sql.="'A',";
            ...
            case 'actualizar_cupo':
                ...
                $cadena_sql.="SET CUR_NRO_INS= (select count(*) from acins where ins_asi_cod = ".$variable[2]." and ins_gr=".$variable[1]." and ins_ano=(SELECT APE_ANO FROM ACASPERI WHERE APE_ESTADO LIKE '%V%') and ins_per=(SELECT APE_PER FROM ACASPERI WHERE APE_ESTADO LIKE '%V%'))";
                $cadena_sql.=" WHERE CUR_APE_ANO=(SELECT APE_ANO FROM ACASPERI WHERE APE_ESTADO LIKE '%V%') ";
                $cadena_sql.="AND CUR_APE_PER=(SELECT APE_PER FROM ACASPERI WHERE APE_ESTADO LIKE '%V%') ";
                $cadena_sql.="AND CUR_ASI_COD=".$variable[2]." AND CUR_NRO=".$variable[1];
