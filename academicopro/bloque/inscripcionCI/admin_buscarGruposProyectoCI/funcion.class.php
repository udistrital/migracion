...
class funcion_adminBuscarGruposProyectoCI extends funcionGeneral {
  ...
  function __construct($configuracion, $sql) {
    ...
    //Datos de sesion
    $this->formulario = "registro_adicionarEspacioEstudianteCoorCI";
  ...
  function presentarHorario($resultado_grupos,$estilo,$enlace,$estudiantes,$variables,$icono) {
    ...
    for ($j = 0; $j < count($resultado_grupos); $j++) {
      if((isset($resultado_grupos[$j-1]['CARRERA'])?$resultado_grupos[$j-1]['CARRERA']:'')!=$resultado_grupos[$j]['CARRERA']&&$_REQUEST['parametro']=='!='){
        $permiso=$this->fechas->validar_fechas_CI_coordinador_otros_grupos($this->configuracion, $resultado_grupos[$j]['CARRERA']);
      }
      if ((isset($resultado_grupos[$j-1]['CARRERA'])?$resultado_grupos[$j-1]['CARRERA']:'')!=$resultado_grupos[$j]['CARRERA']&&$_REQUEST['parametro']=='='){
        $permiso=$this->fechas->validar_fechas_CI_grupo_coordinador($this->configuracion, $resultado_grupos[$j]['CARRERA']);
      }
      if ($permiso=='adicion')
        {
        ...
        $variables['idGrupo'] = $resultado_grupos[$j]['ID_GRUPO'];
  ...
  <table width="100%" align="center" border="0" >
    <tr class="bloquelateralcuerpo">
      <td class="centrar" width="50%">
        ...
        $ruta = "pagina=admin_buscarGruposProyectoCI";
...
//                   }
  ...
  function crearEnlaceHorario() {
    ...
    <td class="centrar" width="50%">
      <?
      $pagina = $this->configuracion["host"] . $this->configuracion["site"] . "/index.php?";
      $variable = "pagina=admin_consultarInscripcionEstudianteCoorCI";
  ...
  ?>
    <td class="centrar" width="50%">
      <?
      $pagina = $this->configuracion["host"] . $this->configuracion["site"] . "/index.php?";
      $variable = "pagina=admin_buscarEspacioEstudianteCoorCI";
  ...
  function crearEnlaceCambioUno($estilo, $disponibles, $cruce, $resultado_grupos, $icono) {
      ...
      ?>
          <form enctype='tipo:multipart/form-data,application/x-www-form-urlencoded,text/plain' method='POST' action='index.php' name='<? echo $_REQUEST['destino'] ?>'>
            ...
            <input type="hidden" name="idGrupo" value="<? echo $resultado_grupos['ID_GRUPO'] ?>">
            ...
            $datos=$_REQUEST;
            $datos['idGrupo']=$resultado_grupos['ID_GRUPO'];
