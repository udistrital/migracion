...
<?php
/**
 * Funcion adminInscripcionGrupoCoorCI
 *
...
/**
 * Clase funcion_adminInscripcionGrupoCoorCI
 *
 * Clase que contiene los métodos que ejecutan tareas y crean los formularios de la pagina del bloque.
 * @package InscripcionCoordinadorPorGrupo
 * @subpackage Admin
 */
class funcion_adminInscripcionGrupoCoorCI extends funcionGeneral {
  ...
  public $planEstudio;
    ...
    /**
     * Método constructor que crea el objeto sql de la clase funcion_adminInscripcionGrupoCoorCI
     *
    ...
    function __construct($configuracion) {
        ...
        $this->formulario = "admin_inscripcionGrupoCoorCI";
        $this->bloque = "inscripcionCI/admin_inscripcionGrupoCoorCI";
    ...
    //Buscar año y periodo activos
      $cadena_sql = $this->sql->cadena_sql("periodo_activo_vacaciones","");
  ...
  function cambiarPlanEstudio() {
      ...
      $ruta="pagina=admin_inscripcionCoordinadorCI";
      $ruta.="&usuario=".$this->usuario;
      $ruta.="&tipoUser=".$this->nivel;
      $ruta.="&opcion=verProyectos";
      if($this->nivel==4 ||$this->nivel==28){
        $ruta.="&modulo=Coordinador";
      }elseif($this->nivel==110){
        $ruta.="&modulo=asistente";
      }elseif($this->nivel==114){
        $ruta.="&modulo=secretario";
      }
            ...
            foreach ($arregloEspacios as $key => $espacio) {
                ...
                foreach ($resultadoGrupos as $key => $grupo) {
                    ...
                    if(is_array($resultadoHorario)){
                        $enlace=$this->mostrarEnlaceGrupo($grupo['ID_GRUPO'],$espacio['CODIGO']);
...
<!--        <tr><td colspan="10" align="center">Seleccione el Nivel</td></tr>-->
    ...
    ?><div style="margin: 6px"><?
        for($a=0;$a<=5;$a++){
                    ...
                    <?echo (isset($rotulos[$a])?$rotulos[$a]:'').'<b>'.(isset($datosEspacio[$a])?$datosEspacio[$a]:'').'</b>'?>
                    </div>
     ...
     */
    function mostrarEnlaceGrupo($idGrupo,$espacio) {
