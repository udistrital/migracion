...
<script language='javascript'>
            ...
            {
                ventanaDestinatarios = window.open(enlace,'Codigos', 'width=700,height=700');
            }

            function pasarNombreDestinatarios(cadena, miFormulario)
            {            
...
/**
 * Clase funcion_mensajeEstudiante
 *
 * Clase para envio de mensaje por parte del estudiante
 * Incluye la funcionalidad de seleccinar destinatarios
 * En la clase funcion_mesnajeUsuario existe una funcionalidad mas ampliada
 * 
 * @package Comunicaciones
 * @subpackage Estudiante
 */
class funcion_mensajeEstudiante extends funcionGeneral {
	 ...
	 *        	Arreglo que contiene todas las variables de configuracion que se encuentre en la tabla de configuracion del framework
	 */
	function __construct($configuracion) {
		...
		$this->formulario = "registro_mensajeEstudiante"; // nombre del bloque que procesa el formulario
...
<form
	...
	method='POST' action='index.php' name='<? echo $this->formulario ?>'
	id="<? echo $this->formulario ?>">
	<table id="tabla" class="sigma ">
		...
		<tr id="fila" class="sigma" class="izquierda">
			<td><a
				href='javascript: seleccionarDestinatarios("<?echo $enlaceDestinatarios?>")'>Para:</a>
			</td>
			<td><textarea rows="1" cols="80" name="para" readonly="readonly"
					disabled="disabled">Por favor seleccione los destinatarios haciendo click en la palabra "Para:"...</textarea>
			</td>
		</tr>
		<tr id="fila" class="sigma" class="izquierda">
			<td>Asunto:</td>
			<td><input type="text" name="asunto" size="70"></td>
		</tr>
		<tr id="fila" class="sigma" class="izquierda">
			<td colspan="2"><textarea rows="10" cols="90" name="contenido"></textarea>
			</td>
		</tr>
	</table>
	<input type="hidden" name="opcion" value="enviarMensaje"> <input
		type="hidden" name="receptor"> <input type="hidden" name="action"
		value="<? echo $this->formulario ?>"> <input class="boton"
		type="button" value="enviar" name="enviar"
		onclick="if(verificarFormulario(<?echo $this->formulario?>)){document.forms['<? echo $this->formulario?>'].submit()}else{false}"><br>
</form>
...
<?
	 ...
	 *
	 * Clase que presenta una ventana emergente con listado de detinatarios para seleccionar
	 * el estudiante unicamente tiene opcion de seleccionar a su docente consejero
	 */
	function seleccionarDestinatarios() {
		$DocenteDestinatario = $this->buscarDocenteDestinatario();
		
		if (!isset($DocenteDestinatario)){
			echo "No tiene consejero asignado. Por favor contacte a su Proyecto Curricular";
		} else {		
...
<form
	...
	<fieldset>
		...
		<input type="checkbox" name="checkbox11" value="checkbox"
			onClick="ChequearTodos(this,'<?echo $this->formulario?>');">&nbsp;Todos
		...
		foreach ( $DocenteDestinatario as $key => $value) {
			
			?>
                        <tr>
				<td><input type="checkbox"
					name="<?echo 'docenteConsejero['.$key.']'?>"
					value="<?echo $value['CODIGO'].','.$value['APELLIDO'].' '.$value['NOMBRE'].','.$value['TIPO']?>">
					<b>
                              <?echo $value['CODIGO']?>
                              </b></td>
				<td><b>
...
// presenta el tipo de estudiante, HORAS o CREDITOS
	...
	<input type="hidden" name="opcion" value="pasarDestinatario"> <input
		type="hidden" name="action" value="<? echo $this->formulario ?>">
...
<?
	...
	function enviarDestinatarios() {
		...
		?>
<script language='javascript'>
            pasarNombreDestinatarios("<?echo $cadenaNombres?>","<? echo $this->formulario ?>")
...
<?
	...
	function buscarDocenteDestinatario() {
		...
		$cadena_sql = $this->sql->cadena_sql ( "buscarDocenteDestinatario", $variablesEstudiante ); // echo $cadena_sql;
		$arreglo_docente = $this->ejecutarSQL ( $this->configuracion, $this->accesoOracle, $cadena_sql, "busqueda" );
		return $arreglo_docente;
	}
	
	/**
	 * convierte la cadena destinatario en un arreglo
	 *
	 * Recibe los paremetros $_REQUEST
	 *
	 * @return <type> Array
	 */
	 ...
	 * armarCadenaNombres
	 *
	 * Se ensambla una cadena con los nombres para pasarla al formulario del mensaje en
	 * el campo "Para:"
	 *
	 *
	 ...
	 * esto para facilitar la separación de los datos con la funcion explode.
	 * 
	...
	function armarCadenaDatosReceptor($datosReceptor) {
		...
		// se debe optimizar este método para contemplar todos los tipos de usuario, el tipo de usuario
		// de estudiante debe llegar con codigo y no con S o N
		foreach ( $datosReceptor as $key => $value ) {
			// echo $key.'=>'.$value['NOMBRE'].'<br>';
	...
	/**
	 * Conviente la cadena $_REQUEST['receptor'] en un arreglo con: CODIGO, TIPO y NOMBRE
	 * para cada receptor
	 *
	 * @return array
	 */
	...
	/**
	 * redireccionarEnviado a la pagina admin_mensajeEstudiante
	 *
	 *
	...
	function reporteEnvio() {
		?>
<div
	style="color: #13120d; background-color: #fff1a8; font-family: vardana; font-size: 16px; text-align: center; width: 250px">
...
<?
	 ...
	 *        	codigo reservado para para la insercion del mensaje
	 * @return <type>
	 */
	...
	function insertarMensajeReceptor($datosReceptor, $codigoMensaje) {
		$variablesMensajeReceptor = array (
				'codigoMensaje' => $codigoMensaje,
