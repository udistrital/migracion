    ...
    function consultarEspaciosPermitidos() {
        ...
        foreach ($this->carreras as $proyectos)
        {
            ...
            if (is_array($this->estudiantes)&&isset($this->estudiantes[0]['CARRERA']))
            {
                    ...
                    if (!is_array($planEstudiosEstudiante))
                    {
                    ...
                    }else
                        {
                            ...
                            if(is_array($espaciosPorCursar))
                            {
                                ...
                                {
                                    echo "Sin espacios por cursar: ".$estudiante['CODIGO']."<br>";
                                    $estudiantesSinEspacios++;
                                    if($estudiante['TIPO_ESTUD']=='N')
                                    {
                                        $parametrosPlanEstudiante=$this->buscarParametrosPlanEstudianteHoras($estudiante['CARRERA']);
                                        $cadenaParametrosPlan=json_encode($parametrosPlanEstudiante);
                ...
                echo "<br>".$proyectos['CARRERA']." - ".$proyectos['NOMBRE_CARRERA'].": ".$numeroEstudiantesProyecto."<br>";
                echo "ESTUDIANTES SIN ESPACIOS POR CURSAR: ".$estudiantesSinEspacios."<br>";                
                $totalEstudiantes+=$numeroEstudiantesProyecto;
    ...
    function buscarEspaciosCursados($codEstudiante,$notaAprobatoria) {
        ...
        if(is_array($this->notas)&&isset($this->notas[0]['PROYECTO']))
        {
            foreach ($this->notas as $nota)
            {
                ...
                {
                    if($nota['NOTA']>=$notaAprobatoria||$nota['OBSERVACION']==19)
                    {
    ...
    function buscarEquivalencias($espaciosPorCursar,$carrera) {
        foreach ($espaciosPorCursar as $key => $espacio)
        {
            foreach ($this->espaciosEquivalentes as $equivalencia) {
                if (((isset($equivalencia['ASI_COD_ANTERIOR'])?$equivalencia['ASI_COD_ANTERIOR']:'')==$espacio['CODIGO']||(isset($equivalencia['ASI_COD_ANTERIOR2'])?$equivalencia['ASI_COD_ANTERIOR2']:'')==$espacio['CODIGO'])&&$equivalencia['CARRERA']==$carrera)
                {
