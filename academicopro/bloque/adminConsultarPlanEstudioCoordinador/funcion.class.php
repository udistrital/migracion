...
class funcion_adminConsultarPlanEstudioCoordinador extends funcionGeneral {
  ...
  private $datosPlan;
    ...
    function __construct($configuracion) {
        ...
        $this->formulario="adminConsultarPlanEstudioCoordinador";
...
<table class="contenidotabla centrar" width="100%" >
    ...
    <?
    if(!is_numeric($this->datosPlan[0][7])&&isset($this->datosPlan[0][7])){
    ?>
...
<?
    ...
    /**
     * Esta funcion permite seleccionar un plan de estudios asociado al coordinador
     * @param <type> $configuracion
     */
    function verProyectos($configuracion) {
        ...
        $cadena_sql_proyectos=$this->sql->cadena_sql($configuracion,"proyectos_curriculares",$this->usuario);
        ...
        if(is_array ($resultado_proyectos)&&count($resultado_proyectos)>1) {
          ...
          <table class='contenidotabla centrar' background="<?echo $configuracion['site'].$configuracion['grafico']?>/escudo_fondo.png" style="background-attachment:fixed; background-repeat:no-repeat; background-position:top">
              ...
              for($i=0;$i<count($resultado_proyectos);$i++) {
                ...
                <tr>
                  <?
                  //presenta los planes de estudios y/o proyectos curriculares asociados al coordinador, si tiene mas de uno
        ...
        }else
          {
            ...
            //si solo tiene asociado un plan de estudio, lo presenta
    ...
    /**
     * Esta funcion presenta mensaje cuando no hay planes de estudio
     * @param <type> $configuracion
     */
    ...
    function mostrarRegistro($configuracion) {
        ...
        #$id es el id_carrera
        $this->datosPlan=$this->consultarDatosPlan($planEstudio);
        $this->encabezadoModulo();
        $this->menuCoordinador($configuracion);
        $this->verPlanEstudios($configuracion);
     ...
     * @param <type> $registro
     */
    function verPlanEstudios($configuracion) {
    ...
    function listaEspacios($niveles) {
      ...
      $this->presentarCreditosRegistradosPlan($this->datosPlan[0]['PLAN']);
    }
    ...
    function propedeutico($nivel,$registroEncabezados,$registroEspacios) {
          ...
          if(is_array($registroEncabezados))
          {
              foreach ($registroEncabezados as $key => $value) {
                  ...
                  if($encabezadoEliminado['ID_ENC']!=$id_encabezado)
                  {
                    $this->mostrarCeldaEncabezado($encabezadoEliminado,$registro);
                    if($encabezadoEliminado['COD_CLASIF_ENC']!=4)
                    {
                      if (!is_null($encabezadoEliminado['ID_ESP']))
                      {
     ...
     * @param <type> $this->datosPlan
     */
    function mostrarEnlaceExtrinsecas() {
      $pagina = $this->configuracion["host"] . $this->configuracion["site"] . "/index.php?";
      $variable = "pagina=adminConsultarElectivosExtrinsecosCoordinador";
      $variable.="&opcion=ver";
    ...
    */
    function verRegistro($configuracion) {
      ...
      $this->listaMallas($this->configuracion, $registro);
    }#Cierre de funcion verRegistro

    /**
     * Esta funcion muestra los planes de estudio asociados al coordnador cuando no tiene o tiene mas de uno
     * @param <type> $configuracion
     * @param <type> $registro
     */
    function listaMallas($configuracion,$registro) {
      $this->menuCoordinador($configuracion,$registro);
      ?>
      <table  class="contenidotabla centrar" width="100%" >
     ...
     * @param <type> $nombreProyecto
     */
    function buscarEnfasis($configuracion, $id, $nombreProyecto)
    {
      ...
      <tr>
        <td class='bloquecentralcuerpo' align="center"><strong>No exiten planes de estudio</strong></td>
          ...
          {
            $this->mostrarRegistroOtroPlan($this->configuracion,$registro[0][0]);
          }
    }
     ...
     * @param <type> $planEstudio
     */
    function mostrarRegistroOtroPlan($configuracion,$planEstudio='') {
        if ($planEstudio==''){
            $planEstudio=$_REQUEST['planEstudio'];
          }else
            {
