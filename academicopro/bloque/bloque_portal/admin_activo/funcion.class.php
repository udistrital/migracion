...
class admin_activo implements funcionRegistro
{
	...
	function nuevoRegistro($configuracion,$tema,$acceso_db)
	{
		...
		$this->nuevoActivo($configuracion,$tema,$cripto);
	  ...
	  //Si no se viene de una hoja anterior
		...
		//Rescatar una hoja específica
				...
				{	
					...
					$cadena="En la actualidad no hay Activos Registrados.";
				...
				else
				{
					...
					$this->lista_activo($configuracion,$registro,$campos,$tema,$this->cripto);
					$menu->menu_navegacion($configuracion,$_REQUEST["hoja"],$hojas,$variable);
				}
	...
	function lista_activo($configuracion,$registro,$campos,$tema,$cripto)
	{ 
	...
	<?
		...
		{	$this->nact=$registro[$contador][2];
			$this->tact=$registro[$contador][3];
			$this->nivel_sql=$this->sql->cadena_sql($configuracion,"select",$this->nact);
			//echo $this->nivel_sql;
			$this->nivel=$this->acceso_db->ejecutarAcceso($this->nivel_sql,"busqueda");
			...
			$this->tipo=$this->acceso_db->ejecutarAcceso($this->tipo_sql,"busqueda");
				?>	
					<tr>
						...
						$variable="pagina=adminActivo";
						$variable.="&opcion=ver";
						$variable.="&id_activo=".$registro[$contador][0];
	...
	function nuevoActivo($configuracion,$tema,$cripto)
	{
		$datos="";
		$contador=0;
		...
		<table align='center' width='100%' cellpadding='7' cellspacing='1'>
			<tr class='bloquecentralcuerpo' onmouseover="setPointer(this, <? echo $contador ?>, 'over', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmouseout="setPointer(this, <? echo $contador ?>, 'out', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmousedown="setPointer(this, <? echo $contador++ ?>, 'click', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');">
				...
				<td bgcolor='<? echo $tema->celda ?>'><?
					...
					$html=new html();
					$busqueda="SELECT id_tipo AS id_nivel,valor FROM ".$configuracion["prefijo"]."variable WHERE tipo='NIVEL_ACTIVO' ORDER BY valor";
					$nivel=$html->cuadro_lista($busqueda,'id_nivel',$configuracion,1,0,FALSE,$tab++);
					echo $nivel;
					?>	
				</td>
			</tr>
			<tr class='bloquecentralcuerpo' onmouseover="setPointer(this, <? echo $contador ?>, 'over', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmouseout="setPointer(this, <? echo $contador ?>, 'out', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmousedown="setPointer(this, <? echo $contador++ ?>, 'click', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');">
				...
				<td bgcolor='<? echo $tema->celda ?>'><?
					include_once($configuracion["raiz_documento"].$configuracion["clases"]."/html.class.php");
					$html=new html();
					$busqueda="SELECT id_tipo,valor FROM ".$configuracion["prefijo"]."variable WHERE tipo='TIPO_ACTIVO' ORDER BY valor";
			...
			<tr class='bloquecentralcuerpo' onmouseover="setPointer(this, <? echo $contador ?>, 'over', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmouseout="setPointer(this, <? echo $contador ?>, 'out', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmousedown="setPointer(this, <? echo $contador++ ?>, 'click', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');">
				...
				<td bgcolor='<? echo $tema->celda ?>'>
					<textarea id='descripcion' name='descripcion' cols='46' rows='3' tabindex='<? echo $tab++ ?>' ></textarea>
				</td>
			</tr>
			<tr align='center'>
				<td colspan='2' rowspan='1'>
					<input type='hidden' name='action' value='admin_activo'>
...
function editar_registro_noticia($configuracion,$registro,$tema,$cripto)
 ...
 function guardarActivo($configuracion)
	{
		/*
		...
		$variable[0]=$_REQUEST["id_nivel"];
		$variable[1]=$_REQUEST["id_tipo"];
		$variable[2]=$_REQUEST["nombre"];
		$variable[3]=$_REQUEST["descripcion"];
		$variable[4]='1228170033';				
		...
		$variable[5]= $recuperado[0][0];
		$this->nivel_sql=$this->sql->cadena_sql($configuracion,"insertar_nivel",$variable);
		$nivel=$this->acceso_db->ejecutarAcceso($this->nivel_sql,"busqueda");	
		$this->tipo_sql=$this->sql->cadena_sql($configuracion,"insertar_tipo",$variable);
		$tipo=$this->acceso_db->ejecutarAcceso($this->tipo_sql,"busqueda");	
		if($resultado==true)
		{
			unset($_REQUEST['action']);		
			$pagina=$configuracion["host"].$configuracion["site"]."/index.php?";
