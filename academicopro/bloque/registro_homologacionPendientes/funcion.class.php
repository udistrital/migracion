...
class funcion_registroHomologacionPendientes extends funcionGeneral {
  //Crea un objeto tema y un objeto SQL.
    ...
    { 
        ...
        $cohorte= isset($_REQUEST['cohorte'])?$_REQUEST['cohorte']:'';     
        if($cohorte>0){
                ...
                if(is_array($estudiantes_pendientes) && count($estudiantes_pendientes)>0){
                        foreach ($estudiantes_pendientes as $key => $value) {
                            $codEstudiante[$key] = $value['COD_ESTUDIANTE'];
                        }
                }
        }
       
        $this->validarRegistroEstudiantes($codEstudiante); 
     ...
     */
    function validarRegistroEstudiantes($estudiantes)
    {         
        $k=0;
