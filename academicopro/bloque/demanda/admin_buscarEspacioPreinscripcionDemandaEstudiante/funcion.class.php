...
class funcion_admin_buscarEspacioPreinscripcionDemandaEstudiante extends funcionGeneral {      //Crea un objeto tema y un objeto SQL.
  ...
  public $sesion;
   ...
   // include ($configuracion["raiz_documento"] . $configuracion["estilo"] . "/basico/tema.php");
    ...
    $this->bloque = "demanda/registro_preinscribirEspacioDemandaEstudiante";
...
<?
    ...
    if (is_array($this->espaciosPorCursar)){
      if (isset($this->espaciosParaInscribir)&&is_array($this->espaciosParaInscribir))
          {
              $this->mostrarEspaciosPorCursar('ESPACIOS PERMITIDOS','Adicionar',$this->espaciosParaInscribir,$parametros,'inscribir');
          }else
            {
  ...
  function consultarDatosEstudiante($codEstudiante){        
   ...
   /**
     ...
     */
    function mostrarEspaciosPorCursar($titulo,$nombreObservacion,$espaciosAMostrar,$parametros,$observacion=''){ 
        include_once($this->configuracion["raiz_documento"] . $this->configuracion["clases"] . "/encriptar.class.php");    
        $ancho ='18%';
        
        //Se comenta para que no se puedan preinscribir espacios equivalentes
        //$this->buscarEspaciosEquivalentes();
        $cancelados=$this->buscarEspaciosCancelados();        
        ?>
        
        <!--        <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.4/jquery.js"></script>-->
        <script src="<?=$this->configuracion["host"].$this->configuracion["site"].$this->configuracion["javascript"]?>/jqueryPI.js"></script>
       ...
       texto='Seleccione un espacio académico';    
            ...
            function enviar()
            {
                ...
                { 
					var texto='Error de datos!! Intenta de nuevo';
					p('#boton_enviar').html('');
					p('#mensaje_texto').html(texto);
					p('#mensaje').fadeIn();
                }
                        ...
                        <?
                            ...
                            {?>
                                <div class='cuadro_espacio' style="width:36%"><?
                                    ...
                                    {
                                        echo "El espacio supera el m&aacute;ximo de semestres consecutivos.<br>";
                                    }
                                ?></div><?
                            }?>
                        </div>
                        <?
                        //La siguiente parte se comenta para que no aparezcan los espacios equivalentes en la preinscripción.
                                ...
                                if($equivalente['CODIGO_ESPACIO']==$espacio['CODIGO'])
                                {
                                    ...
                                    <div class="contenidotabla_equivalencias" onmouseover="this.style.background='#F4FA58'" onmouseout="this.style.background=''">
                                        ...
                                        ?>
                                            <div class='cuadro_plano_centrar' style="width:<?echo $ancho;?>"><? echo $clasificacion; ?></div>
                                            <div class='cuadro_espacio' id="centrar_let"style="width:<?echo $ancho;?>">
                                            <?$equivalente['CODIGO']=$equivalente['CODIGO_EQUIVALENCIA'];
...
function porcentajeParametros(){
    ...
    $this->parametrosOB=$this->parametros['OB']- $OBEst;    
    $OCEst=$cadenaCreditosAprobados['OC']; 
    $this->parametrosOC=$this->parametros['OC']- $OCEst;    
    $EIEst=$cadenaCreditosAprobados['EI']; 
    $this->parametrosEI=$this->parametros['EI']- $EIEst;   
    $EEEst=$cadenaCreditosAprobados['EE']; 
    $this->parametrosEE=$this->parametros['EE']- $EEEst;    
    $CPEst=$cadenaCreditosAprobados['CP']; 
    $this->parametrosCP=$this->parametros['CP']- $CPEst;    
...
function mostrarMensajeDeEspera(){
        ?>
        	<div id="cargando" style="z-index:2;background-color: #ffffff; layer-background-color: #336699; border: 1px double #000000; font-size: 24px; color:#CCCCCC; height:40px; width:600px ">
		        ...
		        <img style="width: 25px;  height: 25px; border-width: 0px;" src="<? echo $this->configuracion["host"].$this->configuracion["site"].$this->configuracion["grafico"] ?>/ajax-loader.gif">
                        <font face="Verdana" size="3" color="#CC0000">
                            <?
                                ...
                                echo " Carga de datos en proceso ... ";
