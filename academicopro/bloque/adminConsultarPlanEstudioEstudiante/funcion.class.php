...
class funcion_adminConsultarPlanEstudioEstudiante extends funcionGeneral {
  ...
  //@ Método costructor que crea el objeto sql de la clase sql_noticia
    function __construct($configuracion) {
        ...
        $this->tema=$tema;
        $this->sql=new sql_adminConsultarPlanEstudioEstudiante();
        $this->log_us= new log();
        $this->parametrosPlan=new planEstudios();
        $this->formulario="adminConsultarPlanEstudioEstudiante";
    ...
    /**
     * Funcion que presenta el menu de opciones del estudiante
    ...
    function mostrarRegistro() {
        ...
        $this->cadena_sql=$this->sql->cadena_sql($this->configuracion,"buscar_planEstudioEstudiante",$codEstudiante);
        $registroPlanEstudiante=$this->accesoOracle->ejecutarAcceso($this->cadena_sql,"busqueda");

        if(is_array($registroPlanEstudiante))
            {
                for($i=0;$i<count($registroPlanEstudiante);$i++)
                {
                    $planEstudio=$registroPlanEstudiante[$i][0];
                    $codProyecto=$registroPlanEstudiante[$i][1];
                }
            }
...
#Funcion que muestra la informacion del Plan de Estudios
      ...
      if (is_array($niveles)){
        foreach ($niveles as $key => $nivel) {
          ...
          if ($nivel[0]==$nivelPropedeutico)
          {
            ...
            //Busca espacios del plan que no esten asociados
            $registroPropedeuticos=$this->consultarPropedeuticos($variablesEspacios);
            if(is_array($registroEncabezadosPropedeuticos) || is_array($registroPropedeuticos))
            {
              $this->propedeutico($nivel,$registroEncabezadosPropedeuticos,$registroPropedeuticos);
            }
          }
        }
      }
    ...
    function mostrarEnlaceExtrinsecas() {
      ...
      $variable = "pagina=adminConsultarElectivosExtrinsecosEstudiante";
      $variable.="&opcion=ver";
   ...
   function verRegistro() {
    ...
    function buscarEnfasis($id, $nombreProyecto)
    {
      ...
      <tr>
        <td class='bloquecentralcuerpo' align="center"><strong>No exiten planes de estudio</strong></td>
          ...
          {
            $this->mostrarRegistroOtroPlan($registro[0][0]);
          }
    }
     ...
     * @param <type> $planEstudio
     */
    function mostrarRegistroOtroPlan($planEstudio='') {
        if ($planEstudio==''){
        ...
        $codProyecto=$this->datosPlan[0]['COD_PROYECTO'];
        $nombreProyecto=$this->datosPlan[0]['PROYECTO_NOMBRE'];
        #Consulta los Espacios Academicos del plan de estudios
        $this->encabezadoModulo();
        $this->menuEstudiante($this->datosPlan);
        $this->verPlanEstudios($this->datosPlan);
        #Consulta los Espacios Academicos del plan de estudios
        $niveles=$this->consultarNiveles($planEstudio);

        #Muestra los niveles de un plan de estudios
        $this->listaEspacios($niveles,$this->datosPlan);
    }
