  ...
  function __construct($configuracion, $sql) {
   ...
   $this->espaciosAcademicos=array();
    ...
    function ejecutarCargaPorFacultad() {
        ...
        //Borra datos en base de datos base
        $variables=array('BASE'=>'_base',
                        'CAMPO'=>'ins_fac_cod',
                        'VALOR_CAMPO'=>$codFacultad);
        ...
        //consulta todos los requisitos registrados
        $this->consultarRequisitos();
        //consulta todos los parametros registrados
        $this->consultarParametros();
        //consulta los espacios cancelados por los estudiantes
        $this->consultarEspaciosCancelados();
        //consulta todas las equivalencias y homologaciones entre espacios registradas
        $this->espaciosEquivalentes=$this->consultarEspaciosEquivalentes();
        $this->espaciosAcademicos=$this->buscarEspaciosAcademicos();
        echo $this->carreras[0]['NOMBRE_FACULTAD']."<br>";
        //ejecuta proceso por cada proyecto
        $p=1;
        foreach ($this->carreras as $proyecto)
        {
            $this->ejecutarCargaPorProyecto($proyecto,$p);
            $p++;
        }
        //borrar datos de base oficial
        $variables=array('BASE'=>'',
                        'CAMPO'=>'ins_fac_cod',
                        'VALOR_CAMPO'=>$codFacultad);
        $borradas=$this->borrarDatosPrecarga($variables);
        //realizar volcado de datos a base oficial
        $carga=$this->cargarDatosTabla($variables);
        //Final del reporte
     ...
     */
    function ejecutarCargaPorProyecto($proyectos,$p=1) {
        
            $this->estudiantesSinEspacios=0;
            $codigoProyecto=$proyectos['CARRERA'];
            //consulta los datos de los estudiantes del proyecto
            $this->estudiantes=$this->consultarDatosEstudiantes($codigoProyecto);
            //busca entre los datos de las carrera las nota aprobatoria del proyecto
            $notaAprobatoria=$this->buscarNotaAprobatoria($codigoProyecto);
            //consulta los espacios de los planes de estudio del proyecto
            $this->espaciosPlanEstudio=$this->consultarEspaciosPlanEstudio($codigoProyecto);
            //escapa caracteres espaeciale sde los nombres de espacios del proyecto
            $this->escaparCaracteresEspacios();
            //consulta las notas de que los estudiantes en A, B, V, J tienen con el proyecto
            ...
            {
            echo "<br>".$proyectos['CARRERA']." - ".$proyectos['NOMBRE_CARRERA'];
            ?>
                <head>
                    <script language="javascript">
                    //Creo una función que imprimira en la hoja el valor del porcentanje asi como el relleno de la barra de progreso
                    function callprogress(vValor,vItem,vTotal,num){
                     document.getElementById("getprogress"+num).innerHTML = 'Cargando datos... '+vItem+' de '+vTotal+' estudiantes.  '+vValor ;
                     document.getElementById("getProgressBarFill"+num).innerHTML = '<div class="ProgressBarFill" style="width: '+vValor+'%;"></div>';
                    }
                    </script>
                    <style type="text/css">
                ...
                <!-- Ahora creo la barra de progreso con etiquetas DIV -->
                 <div class="ProgressBar">
                      <div class="ProgressBarText"><span id="getprogress<?echo $p;?>"></span>&nbsp;% </div>
                      <div id="getProgressBarFill<?echo $p;?>"></div>
                    </div>
                </body><?            
                
                $this->total=0;
                $a=1;
                $numRegistros=count($this->estudiantes);
                foreach ($this->estudiantes as $estudiante)
                {

                    $porcentaje = $a * 100 / $numRegistros; //saco mi valor en porcentaje
                    echo "<script>callprogress(".round($porcentaje).",".$a.",".$numRegistros.",".$p.")</script>"; //llamo a la función JS(JavaScript) para actualizar el progreso
                    flush(); //con esta funcion hago que se muestre el resultado de inmediato y no espere a terminar todo el bucle
                    ob_flush();
                    $a++;
                    //incrementa en uno el numero de estudiantes procesados
                    $numeroEstudiantesProyecto++;
                    //realiza la carga de datos por cada estudiante
    ...
    function ejecutarCargaPorEstudiante($estudiante,$notaAprobatoria) {
                    ...
                    //verifica si el estudiante tiene plan de estudios y si concuerda el tipo de estudiante con el tipo de plan dee studios
                    ...
                    //busca datos adicionales del estudiante en los datos de la carrera
                    ...
                    if (!is_array($planEstudiosEstudiante))
                    {
                        ...
                        {
                            //busca los parametros de inscripcion para el estudiante de horas
                            ...
                            {
                                //busca los parametros de inscripcion para el estudiante de creditos, para el caso es vacio
                        ...
                        //crea el arreglo de datos del estudiante
                        ...
                        //registra el arreglo del estudiante
                            ...
                            if(is_array($espaciosPorCursar))
                            {
                                ...
                                if($estudiante['TIPO_ESTUD']=='N')
                                {
                                ...
                                }else
                                    {
                                        //Busca equivalencias de los espacios por cursar
                                        $equivalencias=$this->buscarEquivalencias($espaciosPorCursar,$estudiante['CARRERA']);
                                        //Filtra los espacios equivalentes que sean de creditos
                                        if(is_array($equivalencias))
                                        {
                                            $equivalencias=$this->buscarEspaciosCreditos($equivalencias);
                                        }
                                        //Crea cadena de espacios equivalentes
                                        $cadenaEquivalencias=json_encode($equivalencias,JSON_UNESCAPED_UNICODE);
//Se realiza ajuste para incluir espacios equivalentes para estudiantes de crṕeditos 05/12/2014
//                                        $cadenaEquivalencias='';
                                        $observacionesNiveles='';
                                        //busca los parametros de inscripcion para el estudiante de creditos
                                        $parametrosPlanEstudiante=$this->buscarParametrosPlanEstudianteCreditos($estudiante['PLAN']);
                                        $cadenaParametrosPlan=json_encode($parametrosPlanEstudiante);
                                        $cadenaCreditosAprobados=json_encode($parametrosEstudiante);
                                    }
                            ...
                            }else
                                {
                                    ...
                                    if($estudiante['TIPO_ESTUD']=='N')
                                    {
                                        ...
                                        {
                                            //busca los parametros para la inscripcion del estudiante en el semestre
                                            $parametrosEstudiante=$this->buscarParametrosEstudiante('',$estudiante['TIPO_ESTUD'],$espaciosAprobados, $estudiante['PLAN']);
                                            //busca los parametros de inscripcion para el estudiante de creditos
                                            $parametrosPlanEstudiante=$this->buscarParametrosPlanEstudianteCreditos($estudiante['PLAN']);
                                            $cadenaParametrosPlan=json_encode($parametrosPlanEstudiante);
                                    ...
                                    //crea arreglo por estudiante
                                    $arregloEstudiante=$this->crearArregloEstudiante($estudiante,$datosAdicionalesEstudiante,'','','',$cadenaParametrosPlan,$cadenaCreditosAprobados,'');
                                    //registra arreglo por estudiante
                                    $this->registrarArregloEstudiantes($arregloEstudiante);
                                }
                        }
                ...
                <tr>                  
                    <td>
                        <form enctype='tipo:multipart/form-data,application/x-www-form-urlencoded,text/plain' method='POST' action='index.php' name='<? echo $this->formulario ?>' id="<? echo $this->formulario ?>">
                            <input type="hidden" name="action" value="preinscripcion/<? echo $this->formulario ?>">
    ...
    */ 
    ...
    /**
     * Funcion que crea un arreglo con información basica de los espacios academicos de la Universidad
     * adicionada el 10/12/2014 Milton Parra
     */
    function buscarEspaciosAcademicos() {
        $espacios=$this->consultarEspaciosAcademicos();
        foreach ($espacios as $key => $espacio)
        {
            $espaciosAcademicos[$espacio['CODIGO']]=array('NOMBRE'=>$espacio['NOMBRE'],
                                                                'CREDITOS'=>$espacio['CREDITOS'],
                                                                'CATEDRA'=>$espacio['CATEDRA'],
    ...
    function buscarRequisitosPlanEstudios($plan,$proyectoEstudiante,$espaciosPorCursar,$espaciosAprobados,$tipoEstudiante){
        switch ($tipoEstudiante){
            case 'S':
                ...
                break;
            case 'N':
                $requisitos=$this->evaluarRequisitosHoras($proyectoEstudiante, $espaciosAprobados, $espaciosPorCursar,$plan);
                break;
            default :
                break;
            }
    ...
    function buscarParametrosEstudiante($espaciosPorCursar, $tipoEstudiante, $espaciosAprobados, $plan) {
        if($tipoEstudiante=='N')
        {
            foreach ($this->espaciosPlanEstudio as $espacio)
            {
                ...
                {                    
                    if ($espacio['CODIGO']==$porCursar['CODIGO'] && $plan==$espacio['PLAN'] && $espacio['NIVEL']!='0' && trim($espacio['ELECTIVA'])=='N')
                    { 
                        $arreglo_niveles[]=$espacio['NIVEL'];                  
                    }
                }
            }
            if(isset($arreglo_niveles)&&is_array($arreglo_niveles)&&!empty($arreglo_niveles))
            {
                sort($arreglo_niveles);
            }else
                {
                    $arreglo_niveles[0]=10;
                }
    ...
    */
    ...
    /**
     * Funcion que filtra de un arreglo de espacios los que son de creditos 
     * @param type $equivalencias
     * @return type
     */
    function buscarEspaciosCreditos($arreglosEspacios) {
        foreach ($arreglosEspacios as $key=>$espacio) {
            if ($this->espaciosAcademicos[$espacio['CODIGO_EQUIVALENCIA']]['CREDITOS']=='N')
            {
                unset($arreglosEspacios[$key]);
            }
        }
        $arreglosEspacios=array_values($arreglosEspacios);
        return $arreglosEspacios;
    }
   ...
   function consultarEspaciosPlanEstudio($codProyecto) {
    ...
    function borrarDatosTablaPrecarga($variables){
        ...
        if ($resultado[0]['TOTAL']<1)
        {
            if ($variables['BASE']=='')
            {
                $base='EN TABLA PRINCIPAL';
            }else
                {
                    $base='EN TABLA BASE';
                }
            echo "NO SE ENCONTRARON DATOS PARA BORRAR ".$base.".<br>";
            $resultado=0;
            return $resultado;
        }else
            {
    ...
    /**
     * Funcion que carga registros de la tabla de carga base a oficial.
     * @return array 
     */
    function cargarDatosTabla($variables){
        $cadena_sql=$this->sql->cadena_sql("cargarDatosBase", $variables);
        $resultado=$this->ejecutarSQL($this->configuracion, $this->accesoGestion, $cadena_sql,"" );
        $afectados=$this->totalAfectados($this->configuracion, $this->accesoGestion);
        if ($afectados<1||$resultado===FALSE)
        {
            ...
            echo "*** NO SE PUDO CARGAR DATOS DE ".$nombre." = ".$variables['VALOR_CAMPO']."***<br>";
            exit;
        }
        return $afectados;
    }
    ...
    */ 
    ...
    /**
    * Funcion que consulta los espacios acdemicos de la Universidad
    */ 
    function consultarEspaciosAcademicos() {
        $cadena_sql = $this->sql->cadena_sql("consultarEspaciosAcademicos", '');
        $resultado = $this->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
        return $resultado;
    }
