    ...
    function mostrarEnlaceGenerarCertificado($configuracion,$identificacion,$valor,$fecha) {
        ...
        <table border="0" width="100%">
            ...
            <td class="centrar subrayado">
                ...
                //$variable.="&action=generarCertificado";
       ...
       function generarCertificado($configuracion){
           //var_dump($_REQUEST);
           echo "<br>funcin generar certificado";
           $tipo_certificado=1;
           $parametro_sql=array('IDENTIFICACION'=>$_REQUEST['identificacion'],
                                'VALOR'=>$_REQUEST['valor_ces'],
                                'FECHA'=>$_REQUEST['fecha_ces']);
           $this->crearCertificado($tipo_certificado,$parametro_sql);
       
       }
   
       function crearCertificado($tipo,$parametro_sql){
           echo "<br>función crear certificado<br><br>";
           ...
           if(is_array($secciones)){
                ...
                /*if (is_array($parametros)){
                    ...
                    $parametros = $this->reemplazarParametrosEnSecciones($parametros,$secciones);
                    $documento = $this->armarDocumento($secciones);
                    echo $documento;
                    //$this->generarPDF($documento);
                }else{
       ...
       function ejecutarSqlParametros($parametros,$parametro_sql){
           ...
           foreach ($parametros as $key => $parametro) {
               $cadena_sql = $parametro[$key]['SENTENCIA_SQL'];
               if($cadena_sql ){
                    $resultado=$this->ejecutarSQL($configuracion, $this->accesoOracle, $cadena_sql, "busqueda");
                    if (is_array($resultado)){
                            if(array_keys($array)>1){
                                $parametros[$key]['VALOR']= $resultado;
                            }else{
       ...
       function reemplazarParametrosEnSql($parametros,$parametro_sql){
           foreach ($parametros as $key => $parametro) {
               $cadena = $parametro[$key]['SENTENCIA_SQL'];
               if($cadena){
       ...
       function reemplazarParametroEnCadena($cadena,$nombre,$valor){
           return $cadena=str_replace($nombre, $valor, $cadena);
           
       }
       
       function verificarValoresConjuntos($parametros){
           foreach ($parametros as $key => $parametro) {
                if(is_array($parametro[$key]['VALOR'])){
                    $arreglo_valores=$parametro[$key]['VALOR'];
                    foreach ($arreglo_valores as $key2 => $value) {
                                $nombre_parametro = $key2;
                                $valor_parametro = $value;
                                foreach ($parametros as $key3 => $value3) {
                                    if($nombre_parametro==$parametros[$key3]['NOMBRE']){
       ...
       function reemplazarParametrosEnSecciones($parametros,$secciones){
           foreach ($parametros as $key => $parametro) {
               $nombre_parametro = $parametro[$key]['NOMBRE'];
               $valor_parametro = $parametro[$key]['VALOR'];
               if($nombre_parametro){
                    foreach ($secciones as $key2 => $value) {
                            $nombre_parametro = "P[".$nombre_parametro."]";
                            $cadena=$secciones[$key2]['CONTENIDO'];
       ...
       function armarDocumento($secciones){
            ...
            foreach ($secciones as $key => $seccion) {
                $documento .= "<br>".$seccion['CONTENIDO'];
            }
            return $documento;
       }
     * 
