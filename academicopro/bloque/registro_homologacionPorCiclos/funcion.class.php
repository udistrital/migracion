...
class funcion_registroHomologacionPorCiclos extends funcionGeneral {
  //Crea un objeto tema y un objeto SQL.
    ...
    //include ($configuracion["raiz_documento"].$configuracion["estilo"]."/".$this->estilo."/tema.php");
        ...
        //Datos de sesion
        $this->formulario="admin_homologacionPorCiclos";
     ...
     */
    function validarRegistroHomPorCiclos()
    {
        $tipo_homologacion= (isset($_REQUEST['tipo_homologacion'])?$_REQUEST['tipo_homologacion']:'');
        switch ($tipo_homologacion) {
            case 'estudiantes':
                $this->validarRegistroEstudiantes($_REQUEST['codEstudiante'],$_REQUEST['codEstudianteAnt']);
     ...
     */
    function validarRegistroEstudiantes($estudiantes,$estudiantesAnt)
    {   
        $k=0;
                ...
                if($cantidadEstudiantes>0){
                    ...
                    $resultadoBifurcacion='';
                        foreach ($estudiantes as $key => $cod_estudiante) {
                            ...
                            $cod_estudianteAnt = $estudiantesAnt[$key];
                            $validar_estudiante_ant = $this->verificarEstudiante($cod_estudianteAnt,'');
                            //verifica que los codigo de estudiante sean validos
                            if($validar_estudiante=='ok' && $validar_estudiante_ant=='ok' )
                            {
                                $datosEstudiante= $this->consultarDatosEstudiante($cod_estudiante,$cod_proyecto); 
                                $datosEstudianteAnt= $this->consultarDatosEstudiante($cod_estudianteAnt,''); 
                               //consulta notas actuales del estudiante
                                ...
                                $estudiantesNoValidos[$k]['cod_estudianteAnt']=$cod_estudianteAnt;
                                $k++;
                            }
                        }
  ...
  function verificarEstudiante($cod_estudiante,$cod_proyecto) {
    ...
    function consultarEspaciosProyecto($cod_proyecto){
      ...
      //echo "<br>sql espacio ".$cadena_sql ;exit;
