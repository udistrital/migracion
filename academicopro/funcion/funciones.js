...
function muestraCalendario(raiz,formulario_destino,campo_destino,mes_destino,ano_destino){
	...
	ventanaCalendario = window.open(raiz+"/calendario/index_cal.php?formulario=" + formulario_destino + "&nomcampo=" + campo_destino,"calendario","width=230,height=250,left=420,top=380,scrollbars=no,menubars=no,statusbar=NO,status=NO,resizable=NO,location=NO")
...
function muestra_div_oculto(elemento,div) {
...
function seleccionCheck(valor,formulario,seleccion)
    {
        var total=parseInt(seleccion)+1;
        //alert(parseInt(seleccion)+2);
        for (var i=0;i < document.forms[formulario].elements.length;i++)
            {
                //alert(document.forms[1].elements[2].type);
                var elemento = document.forms[formulario].elements[i];
         ...
         //for (var i=2;i < document.forms[formulario].elements.length;i++)
         for (var j=2;j <= total;j++)
            {
                //alert(document.forms[1].elements[2].type);
                var elemento = document.forms[formulario].elements[j];
                //alert(elemento.type);
                if (elemento.type == "checkbox")
                    {
                        elemento.click();
...
/**************************************************************************************
 * Muestra un mensaje de error si el tamaño de la cadena es superior a un número dado *
 **************************************************************************************/

function longitudCadenaSuperior(formulario,control,tamanno,nom_campo)
{
    ...
    if (longitud>tamanno) {
        campo.select();
	alert1('Dato incorrecto. El campo " '+nom_campo+' " debe contener un dato con menos de '+tamanno+' caracteres');
        campo.focus();
	return false;
    } 

    return true;
} // Fin de la función longitudCadenaSuperior

/**************************************************************************************
 * Muestra un mensaje de error si la cadena no es numérica con el nombre del campo    *
 **************************************************************************************/

function verificarNumeroNomCampo(formulario,control,nom_campo)
{
    var campo = formulario.elements[control];
    ...
    if(isNaN(campo.value))
     {
        //formulario.reset();
        campo.select();
        alert1('Debe ingresar un valor numérico en el campo '+nom_campo);
        campo.focus();
        return false;
    }
...
/*************************************************************
*Funcion que solo permite el ingreso de datos alfanumericos sin caracteres especiales
**************************************************************/
function soloNumerosYLetras(e) {
key = e.keyCode || e.which;
tecla = String.fromCharCode(key).toLowerCase();
letras = " áéíóúabcdefghijklmnñopqrstuvwxyzABCDEFGHIJKLMNÑOPQRSTUVWXYZÁÉÍÓÚ1234567890";
especiales = [8,9,127];
