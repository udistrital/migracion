            ...
            if($U_resultado)
                 {$cuposValue=$cupos;
                  $msnj="<font color=green>El registro se actualizó correctamente!</font>";
                    if($C_resultado[0]['MAX_CAPACIDAD']<$cupos)
                    {
                        $cuposValue=$C_resultado[0]['CUPOS'];
                        $msnj.="<br><font color=red>¡Atención! El cupo es mayor a la capaciad Máxima del curso.</font>";
                    }      
                 }
            else
                 {$cuposValue=$C_resultado[0]['CUPOS'];
