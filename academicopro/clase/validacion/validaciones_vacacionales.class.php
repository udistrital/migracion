    ...
    if (is_numeric($datosInscripcion['codEstudiante'])) {
        ...
        if (is_array($datosEstudiante)) {
                ...
                for ($i = 0; $i < count($proyectosCoordinador); $i++) {
                    ...
                    if (  trim($datosEstudiante[0]['INDICA_CREDITOS']) == trim($proyectosCoordinador[$i]['CREDITOS'])) {
                        $tipo = 1;
                        $valor = $i;
                    }
                }
...
public function proyectosAsistente() {
  	$cadena_sql_proyectos = $this->cadena_sql("proyectos_curriculares_asistente", $this->usuario);
  	return $resultado_proyectos = $this->funcionGeneral->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql_proyectos, "busqueda");
  }
  ...
  public function cadena_sql($tipo, $variable) {
    ...
    case 'proyectos_curriculares_asistente':
      ...
      case 'horario_grupo':
        ...
        $cadena_sql.=" INNER JOIN accursos ON horario.hor_id_curso=accursos.cur_id";
      ...
      case 'horario_estudiante':
        ...
        $cadena_sql.=" FROM achorarios horario";
        $cadena_sql.=" INNER JOIN accursos ON horario.hor_id_curso=accursos.cur_id";
        $cadena_sql.=" INNER JOIN acins ON cur_asi_cod=acins.ins_asi_cod AND cur_id=acins.ins_gr";
        $cadena_sql.=" AND cur_ape_ano=acins.ins_ano AND cur_ape_per=acins.ins_per";
