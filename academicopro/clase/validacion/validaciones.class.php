...
class validarInscripcion {
  ...
  public function validarEstudiante($datosInscripcion) {
    ...
    if (is_array($datosEstudiante)) {
    	...
    	//Swith para seleccionar si es coordinador o asistente
    	//var_dump($_REQUEST);
    	//$this->consultarProyectosAsistente($this->usuario,110);
    	//exit;
    	switch($this->nivel){
			case '110':
				$proyectosCoordinador = $this->proyectosAsistente();
   ...
   * @return string
   */  
  public function validarProyectoAsistente($codEstudiante, $codAsistente)
    {
        if(is_numeric($codEstudiante))
        {
          ...
          if(is_array($resultado_estudiante))
            {
                $mensaje=$this->verificarProyectoAsistente($resultado_estudiante[0][1],$codAsistente,$this->nivel);
     ...
     * @return string
     */   
    function verificarProyectoAsistente($codProyecto,$codAsistente,$tipoUsuario){
                $resultado_proyectos = $this->consultarProyectosAsistente($codAsistente,$tipoUsuario);
                //var_dump($resultado_proyectos);
                $tipo=0;
     ...
     * @return type
     */
    function consultarProyectosAsistente($codAsistente,$tipoUsuario){
  ...
  /**
   * Esta funcion permite verificar si el estudiante pertenece a la facultad del asistente y el contrato del asistente esta vigente
   * @param <int> $codEstudiante
   * @param <int> $codAsistente
   * @return string
   */  
  public function validarFacultadAsistente($codEstudiante, $codAsistente)
    {
        if(is_numeric($codEstudiante))
        {
          $cadena_sql=$this->cadena_sql("buscarInfoEstudiante",$codEstudiante);
  ...
  /**
   * Esta funcion permite verificar si el estudiante pertenece a la facultad del secretario academico
   * @param <int> $codEstudiante
   * @param <int> $codSecAcademico
   * @return string
   */  
  public function validarFacultadSecAcademico($codEstudiante, $codSecAcademico)
    {
        if(is_numeric($codEstudiante))
        {
          $cadena_sql=$this->cadena_sql("buscarInfoEstudiante",$codEstudiante);
          ...
          if(is_array($resultado_estudiante))
            {
                ...
                for($i=0;$i<$total;$i++)
                {
                    if($resultado_estudiante[0]['FACULTAD']==$resultado_proyectos[$i]['DEPENDENCIA'])
                    {
                        $mensaje='ok';
                    }
                }
  ...
  public function validarEstadoEstudiante($datosInscripcion) {
    ...
    if (is_null($datosInscripcion['codEstudiante'])) {
      ...
      if (trim($resultadoEstadoEstudiante[0]['CODIGO_ESTADO']) == 'A' || trim($resultadoEstadoEstudiante[0]['CODIGO_ESTADO']) == 'B') {
        return 'ok';
      } else {
    ...
    if (is_array($resultadoEspacioInscrito) && $resultadoEspacioInscrito[0][0] == 0) {
      ...
      if (is_array($resultadoEspacioInscrito)) {
        ...
        if (is_array($datosEspacioInscrito)) {
          ...
          'No se pueden rescatar los datos del espacio ya inscrito';
          exit;
        }
      } else {
        'Fallo validar "espacio ya inscrito"';
        exit;
      }
    }
  }
  ...
  public function validarEspacioPlan($datosInscripcion) {
    ...
    if (is_array($resultadoEspacioPlan) && $resultadoEspacioPlan[0][0] > 0) {
      ...
      if (is_array($resultadoEspacioPlan)) {
        ...
        'Fallo validar "Espacio plan de estudios estudiante"';
        exit;
      }
    }
  }
   ...
   * @return int
   */
    public function verificarCreditosClasificacion($parametros,$espaciosInscritos,$datosEspacio,$aprobados,$plan="") {
        $inscribir='ok';
        $creditos=0;
        if(is_array($espaciosInscritos))
        {
...
<?
  ...
  public function proyectosAsistente() {
  	$cadena_sql_proyectos = $this->cadena_sql("proyectos_curriculares_asistente2", $this->usuario);
  	return $resultado_proyectos = $this->funcionGeneral->ejecutarSQL($this->configuracion, $this->accesoOracle, $cadena_sql_proyectos, "busqueda");
  }
  ...
  public function buscarCupo($datosGrupo) {
    ...
    if (is_array($resultado_cupo) && is_numeric($resultado_cupo[0]['CUPO'])) {
      ...
      "No es posible obtener el cupo del grupo";
      exit;
    }
  }
  ...
  public function buscarInscritos($datosGrupo) {
    ...
    if (is_array($inscritos) && is_numeric($inscritos[0]['INSCRITOS'])) {
      ...
      "No es posible obtener el numero de inscritos del grupo";
      exit;
    }
  }
  ...
  public function cadena_sql($tipo, $variable) {
    ...
    case 'buscarCodigoEstudiante':
      ...
      case 'proyectos_curriculares':
        ...
        case 'proyectos_curriculares_asistente2':
    ...
    break;
      ...
      case 'horario_grupo':
        ...
        $cadena_sql.=" INNER JOIN accursos curso ON curso.cur_id=hor.hor_id_curso ";
      ...
      case 'horario_estudiante':
        ...
        $cadena_sql.=" INNER JOIN acins ON ins_gr=hor_id_curso";
      ...
      case 'consultar_espacioComoExtrinseco':
        ...
        //$cadena_sql.=" AND clp_pen_nro= " . $variable['planEstudio'];//comentada 020814 por solictud milton
...
//                        $cadena_sql.=" AND not_nota>=(SELECT fua_nota_aprobatoria(".$variable['codProyecto'].") FROM dual)";
	...
	$cadena_sql.=" AND not_cra_cod=".$variable['codProyectoEstudiante'];//adicionado 020814 por instrucción milton
        $cadena_sql.=" AND not_nota>=(SELECT cra_nota_aprob FROM accra WHERE cra_cod=" . $variable['codProyectoEstudiante'] . ")";
        $cadena_sql.=" AND not_est_reg LIKE '%A%'";
        break;
