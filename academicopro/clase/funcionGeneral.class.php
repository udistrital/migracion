...
class funcionGeneral
{
	...
	public function ejecutarSQL($configuracion, $conexion, $cadena_sql, $tipo)
	{
    	...
    	public function totalRegistros($configuracion, $conexion) //cuenta los registros de una busqueda
	{
	...
	public function datosGenerales($configuracion,$conexion, $tipo, $variable="") //rescata datos de las sesiones del usuario
	{
	...
	public function rescatarValorSesion($configuracion, $conexion, $tipo)
	{
        ...
        public function guardarValorSesion($configuracion, $conexion, $tipo,$valor)
	{
		include_once($configuracion["raiz_documento"].$configuracion["clases"]."/sesion.class.php");
		$this->sesion=new sesiones($configuracion);
		$this->sesion->especificar_enlace($conexion->obtener_enlace());
		...
		//Rescatar el valor de la variable usuario de la sesion
		$resultado=$this->sesion->guardar_valor_sesion($configuracion,$tipo,$valor,$this->esta_sesion);
	...
	public function cargarArchivoServidor($configuracion,$parametro,$tipoArchivo)
	{
		...
		$subir->unico=$parametro["nombreUnico"];
		//$subir->unico=TRUE;
		$subir->permisos=0777;
		...
		if($resultado==false)
		{
			$this->errorCarga["noCarga"]=$subir->log["resultado"][0]."<br>El archivo no pudo ser cargado en el servidor.";
			return false;
		}
		else
		{
