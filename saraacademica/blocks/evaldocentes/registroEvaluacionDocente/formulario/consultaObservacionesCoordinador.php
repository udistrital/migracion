...
$registroCoordinadores = $esteRecursoDB->ejecutarAcceso($cadena_sql, "busqueda");
    ...
    switch($accion)
    {
            case 4: //Evaluación Por Coordinadores
                  ...
                  {
                     $tipo = 'message';
                     $mensaje = "Lista de docentes de su proyecto curricular con observaciones relazadas por los estudiantes en la evaluación docente, de los peridos 2014-1 en adelante.";
                    ...
                    echo "<thead>
                        ...
                        for($i=0; $i<=count($registroCarreras)-1; $i++)
                        {
                            if(!is_array($registroCarreras))
                            {
                                $carreras=$carrera.",". $registroCarreras[$i][0];
                                $variable['carrera']=$carreras;
                        ...
                        $tipoEvaluacion="1,4";
                        $variable['tipoEvaluacion']=$tipoEvaluacion;
                            
                        $cadena_sql = $this->sql->cadena_sql("consultarDocentesConObservaciones", $variable);
                        $registroDocentesConObservaciones = $esteRecursoDBPG->ejecutarAcceso($cadena_sql, "busqueda");
                          
                        for($j=0; $j<=count($registroDocentesConObservaciones)-1; $j++)
                        {
                            if($registroDocentesConObservaciones[$j][0]!="")
                            {    
                                $docente=$registroDocentesConObservaciones[$j][0];
                                $cadena_sql = $this->sql->cadena_sql("docentes", $docente);
                                ...
                                $variable.="&opcion=observacionesDocentesPorEstudiantes";
                                $variable.="&tipo=".$_REQUEST['tipo'];
                                $variable.="&usuario=".$_REQUEST['usuario'];
                                $variable.="&consulta=coordinador";
                                $variable.="&docente=".$registroDocentesConObservaciones[$j][0];
                                $variable.="&carrera=".$carreras;
                                $variable.="&tipoEvaluacion=".$tipoEvaluacion;
                                ...
                                echo "<tr> ";
                                        echo "<td><a href='".$variable."'>".$registroDocentesConObservaciones[$j][0]."</a></td>";
                                        echo "<td>".$registroDocentes[0][1]."</td>
                                        <td>".$registroDocentes[0][2]."</td>
                            ...
                            {
                                $tipo = 'information';
                                $mensaje = "No hay registro de docentes con observaciones por parte de los estudiantes. ";
